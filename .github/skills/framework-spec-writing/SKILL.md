...
- **シングルトン管理**: `singletons.py` への登録が必要な場合は明記
...
- グローバルインスタンスは `singletons.py` に集約する
- `reset_for_testing()` で全シングルトンをリセット可能にする
- テスト間の状態汚染を防ぐため、フィクスチャで `reset_for_testing()` を呼ぶ

### 後方互換性

- 公開 API の **シグネチャ変更** は破壊的変更として扱い、移行ガイドを仕様書に明記する
- 非推奨 API は `warnings.warn()` で段階的に廃止する（即削除しない）
- 内部実装の変更は、公開 API が同じ振る舞いを保つ限り自由に行える
...
3. **関連ドキュメント確認**: `docs/` 配下の設計ドキュメントを参照する
   - [docs/architecture.md](../../../docs/architecture.md) — 全体アーキテクチャ
   - [docs/logging_design.md](../../../docs/logging_design.md) — ロガー設計
   - [docs/protocol_design.md](../../../docs/protocol_design.md) — プロトコル設計
   - [docs/macro_design.md](../../../docs/macro_design.md) — マクロ基盤設計
   - [docs/notification_system.md](../../../docs/notification_system.md) — 通知システム設計
