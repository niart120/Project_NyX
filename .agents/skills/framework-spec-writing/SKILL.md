...
| LoggerPort | Runtime から注入されるログ出力 port。ユーザ向け event と technical log を分離する |
| MacroRuntimeBuilder | Registry と各種 port factory から実行 context を組み立てる composition root |
...
| パッケージ | 主要クラス / 関数 | 責務 |
|------------|-------------------|------|
| `core/macro/` | `MacroBase`, `Command`, `DefaultCommand`, `MacroRegistry` | マクロ公開 API と登録 |
| `core/runtime/` | `MacroRuntime`, `MacroRuntimeBuilder`, `MacroRunner`, `RunResult` | 実行 context と lifecycle 制御 |
| `core/io/` | `ControllerOutputPort`, `FrameSourcePort`, `ResourceStorePort`, `RunArtifactStore` | Runtime の入出力 port と resource I/O |
| `core/hardware/` | `SerialComm`, `DeviceDiscoveryService`, `CH552SerialProtocol`, `ThreeDSSerialProtocol` | ハードウェア抽象化 |
| `core/imgproc/` | `ImageProcessor`, `TemplateMatcher`, `OCRProcessor` | 画像処理・OCR |
| `core/logger/` | `LoggerPort`, `DefaultLogger`, `LogSinkDispatcher`, `JsonlLogBackend` | 構造化ログと sink dispatch |
| `core/settings/` | `GlobalSettings`, `SecretsSettings` | 設定永続化 |
| `core/notifications/` | `NotificationHandler`, `DiscordNotification`, `BlueskyNotification` | 外部通知 |
| `core/utils/` | `CancellationToken` | キャンセル制御 utility |
