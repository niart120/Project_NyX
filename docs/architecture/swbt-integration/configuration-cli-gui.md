...
| 接続操作 | yes | 左にペアリング／キャンセル、右に接続／切断／キャンセルを配置 |
...
設定画面はタイプ→デバイス→操作段の順に配置する。操作段には行ラベルを置かず、ペアリングと接続の2ボタンを右寄せでまとめ、入力欄の右端にそろえる。ボタンの間に伸縮余白は入れない。設定画面と接続メニューは同じ操作判定を使う。

未接続時は左が「ペアリング」、右が「接続」（プロファイルがある場合のみ有効）。ペアリング中は左、再接続中は右を「キャンセル」に切り替え、他方を無効にする。キャンセル要求後は「キャンセル」のまま無効にする。接続済みなら左を無効にし、右を「切断」に切り替える。切断処理中は両方を無効にする。
