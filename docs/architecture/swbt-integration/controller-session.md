...
`SwbtControllerOutputPortFactory` は session key ごとに active port を 1 つだけ持つ。`create()` で新しい port を作る前に旧 active port を close し、`pair()` / `reconnect()` / `disconnect()` / `close()` / 接続失敗時の session 破棄でも active port を残さない。
