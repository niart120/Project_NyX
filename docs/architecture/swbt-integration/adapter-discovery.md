...
| discovery error | 利用者向け status 表示 + technical log |

候補が 1 件でも combo box の先頭を自動選択しない。保存済み値が候補の `name` または一意な `alias` に一致した場合だけ代表 `name` を選択値にする。refresh が失敗した場合は、保存済み adapter と refresh 前の選択を維持する。

列挙は Qt worker thread で実行し、結果による combo box 更新と error 表示だけを main thread で行う。
