...
| `SwbtControllerSession` | `swbt-python` controller の open、pair/reconnect、apply/neutral/close と例外変換を同期 port から扱うために必要 |
...
`SwbtControllerSession` は上位 domain service ではなく backend resource adapter として扱う。serial backend における `SerialComm` と `SerialProtocolInterface` の役割を、swbt controller lifecycle 向けにまとめたものと位置づける。
...
[ ] GUI thread が swbt controller lifecycle の詳細を直接扱っていない
