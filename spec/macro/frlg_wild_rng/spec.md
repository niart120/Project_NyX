...
| **timer1** | 初期 Seed 決定後からあまいかおり実行までを管理するタイマー（= advance タイマー）。frame2 タイマーの基準時刻とも共有 |
| **frame2** | 初期 Seed 決定から「つづきからはじめる」までの待機フレーム数（マクロ内定数: 300F = 5.000s）。初期 Seed 決定直後の advance を安定させるためのタイミング制御 |
| **timer_teachy** | おしえテレビの起動〜終了間の待機を管理するタイマー。おしえテレビ使用時のみ有効。`teachy_tv_consumption` から逆算したフレーム数を元に操作タイミングを制御する |
| **奇数ズレ補正** | 図鑑を 2 回開閉することで乱数消費の偶奇を調整する操作。メニュー→図鑑開→閉→開→閉→メニュー閉じるの 6 操作で構成される。`use_odd_correction = true` 時のみ実行 |
...
乱数調整の核心は **3 つ（+ オプションで 1 つ）のタイマー**による時間制御である。
...
- **frame2 タイマー**: 初期 Seed 決定 → 「つづきからはじめる」
  - `wait = _FRAME2 / fps` (マクロ内定数: 300F = 5.000s)
  - timer1 と基準時刻を共有し、初期 Seed 決定直後の advance を安定させる
- **timer_teachy** (オプション): フィールド操作可能 → おしえテレビ終了
  - `teachy_frames = (teachy_tv_consumption - teachy_tv_transition_correction) / teachy_tv_adv_per_frame`
...
[ゲーム再起動]
  ...
  ├─ ... timer0 消化 ...
  │
  ├─ ★ timer1 開始 (timer0 消化完了直後) = frame2 基準時刻
  ├─ A (OP スキップ)
  ├─ ... frame2 タイマー消化 (300F = 5.000s) ...
  ├─ A (つづきからはじめる)
  ├─ B (回想スキップ)
  ├─ ── フィールド操作可能 ──
  │
...
| パラメータ | 型 | デフォルト | 説明 |
|------------|-----|-----------|------|
| `use_odd_correction` | `bool` | `false` | 奇数ズレ補正を使用する場合 `true`。図鑑を 2 回開閉して乱数消費の偶奇を調整する |

> **奇数ズレ補正の仕組み**:
>
> GBA の乱数調整では、フィールド上の乱数消費が 2 adv/F（Switch）で偶数単位に進むため、
> 目標 advance が奇数の場合にズレが生じることがある。
> これを補正するために、メニューから図鑑を 2 回開閉する操作を挿入する。
> 図鑑の開閉により奇数回の乱数消費が発生し、偶奇を揃えることができる。
>
> この操作はおしえテレビの前（おしえテレビ未使用時はあまいかおり選択前）に実行される。
> timer1 の管理下で行われるため、操作に費やした壁時計時間は自動的に吸収される。
...
> **1 回実行**: Step 1 〜 Step 8 を順に実行し、あまいかおり発動後にマクロ終了。
...
`use_odd_correction = true` の場合のみ実行する。
メニューから図鑑を 2 回開閉することで乱数消費の偶奇を調整する。
...
> 進むため、目標 advance が奇数の場合に到達できない場合がある。
> 図鑑の開閉で奇数回の乱数消費を発生させ、偶奇のズレを補正する。
> この操作は timer1 の管理下で行われるため、操作に費やした壁時計時間は自動的に吸収される。
>
> おしえテレビ使用時はおしえテレビの前に、未使用時はメニュー操作（あまいかおり選択）の前に実行される。
...
> Step 3 〜 Step 6 で費やした時間（奇数ズレ補正・おしえテレビの実時間含む）は timer1 に自然吸収されるため、
> ここでは残り時間のみを消化する。
...
| ユニット | `test_config_odd_correction_disabled` | `use_odd_correction=false` がデフォルトであること |
| ユニット | `test_config_odd_correction_enabled` | `use_odd_correction=true` が正しく読み込まれること |
...
| 実機 | `test_wild_rng_with_odd_correction` | 奇数ズレ補正ありでエンカウントまでの操作が正常に動作すること |
