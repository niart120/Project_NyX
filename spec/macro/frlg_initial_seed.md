...
| `min_advance` | `int` | `741` | Seed逆算時の LCG advance 探索下限（閉区間） |
| `max_advance` | `int` | `749` | Seed逆算時の LCG advance 探索上限（閉区間） |
...
A    (dur=0.10, wait=0.50)  # バッグを開く
                             # ボールポケットへ移動・マスターボールを選択
A    (dur=0.10, wait=0.50)  # マスターボールを使う
A    (dur=0.10, wait=5.00)  # 捕獲アニメーション → 「ルギアを つかまえた！」
```

> **Note**: 上記はマスターボール使用を前提とした概略シーケンス。
> バッグ内のポケット・カーソル位置に依存するため、実装時にセーブデータの状態に合わせて調整が必要。
> ボタンシーケンス・ウェイトは実機計測で確定する。

### Step 6: 捕獲後操作 → ステータス画面を開く

```
# 捕獲後のダイアログを処理
A    (dur=0.10, wait=1.00)  # 図鑑登録画面を送る（初回捕獲時）
B    (dur=0.10, wait=0.50)  # 「ニックネームを つけますか？」 → B で辞退
A    (dur=0.10, wait=2.00)  # 戦闘終了 → フィールドに戻る
...
- OCR で性格を画像認識（→ §6.2）
- 認識失敗、または性格テーブルに存在しない → `frame2 += 2` で **Step 1 に戻る**
...
- いずれか認識失敗 → `frame2 += 2` で **Step 1 に戻る**

### Step 9: 初期Seed逆算

- 性格 + 実数値から初期Seedを逆算（→ §7）
- 結果:
  - Seed特定成功 → frame2 を元の値にリセット
  - `"False"`（見つからない） → `frame2 += 2` で **Step 1 に戻る**
  - `"MultipleSeeds"`（複数候補） → `frame2 += 2` で **Step 1 に戻る**
...
性格またはステータスの画像認識に失敗した場合、以下のフォールバック処理が行われる:

```
frame2 += 2
→ Step 1（ソフトリセット）に戻ってリトライ
```

Seed逆算に成功した場合は、frame2 を **元の値（設定値）にリセット** する。
これにより、一時的なキャプチャタイミングのズレを自動補正する。
...
- **出力**: 一致候補が 1 つなら 4桁 HEX の Seed / 0 なら `"False"` / 2 つ以上なら `"MultipleSeeds"`

Seed逆算に成功した場合は frame2 を元の値にリセットし、失敗時は `frame2 += 2` でリトライする（→ §5）。
...
1フレーム = 1行。各フレームについて `trials` 回分の Seed 測定結果を記録する（文字コード: UTF-8）。

出力先: `{output_dir}/{file_name}.csv`

```csv
frame,sound,button_mode,seed_1,seed_2,seed_3,seed_4,seed_5
2090,モノラル,ヘルプ,A3F1,A3F1,A3F1,A3F1,A3F1
2091,モノラル,ヘルプ,7C02,False,7C02,7C02,7C02
2092,モノラル,ヘルプ,1234,,,
```
...
| `seed_1` … `seed_5` | `str` | 各測定回の結果。`XXXX`（4桁 HEX）/ `False` / `MultipleSeeds` / 空欄（未測定） |
