...
| `has_save_data` | `bool` | `True` | セーブデータの有無。メニュー選択操作 (↓ の要否) とメッセージ速度係数に影響 |
| `message_wait_factor` | `float` | `has_save_data` 依存 | 会話送り wait の倍率。省略時: `has_save_data=True` → 1.0 / `False` → 1.5 |
...
- `A` → 0.167s → `A` → 3.350s
- **`has_save_data=True`**:
  - `↓` → `A` （メニュー2番目の「さいしょからはじめる」を選択）
- **`has_save_data=False`**:
  - `A` （唯一の選択肢「さいしょからはじめる」を選択）
- 「そうさせつめい」: `A` × 3 (暗転あり)
- 「ゲームのもくてき」: `A` × 3
- オーキド博士のイントロへ
