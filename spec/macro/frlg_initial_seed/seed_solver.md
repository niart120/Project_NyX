...
> **関連マクロ**: FRLG 初期Seed特定マクロ (`spec.md`)  
...
| `"False"` | 一致する候補が見つからなかった | そのまま記録し次の trial へ |
| `"MULT"` | 候補が2つ以上あり一意に絞れなかった | そのまま記録し次の trial へ |

> `"False"` / `"MULT"` が返る主な原因は、画像認識の誤り、またはタイマーのズレ。
> いずれも Seed 逆算結果としてそのまま CSV に記録される（→ `spec.md` §5）。
...
| solve_initial_seed (異常系) | 不正な実数値 → `"False"` / 複数候補 → `"MULT"` |
