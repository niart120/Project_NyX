...
| 項目 | 現状 | 修正方針 |
|------|------|----------|
| Builder 生成 | 実行ごとに `create_legacy_runtime_builder()` を呼ぶ | GUI application lifetime の service に集約し、起動時と設定変更時だけ再構成する |
| Macro identity | `MacroCatalog.macros` が `definition.class_name` を key にしている | stable `definition.id` を実行 ID にし、表示名と分離する |
| capture device 切り替え | PreviewPane が古い active capture device を参照したままになり得る | 設定変更時だけ preview timer を停止し、device 差し替え後に再開する |
...
| close timeout | `wait(5)` が固定値 | `global_settings["runtime.release_timeout_sec"]` から読み、未設定時は `5` 秒にする |
...
```text
MainWindow
  -> GuiAppServices(cached runtime_builder, registry, logging, device services)
...
| Phase 3 | `PHASE_3_PREVIEW_AND_OBSERVABILITY.md` | preview の device 切り替えと `UserEvent` 表示を整理する | Phase 2 |
...
### 3.5 Preview と capture device 切り替え

Runtime 実行中という理由だけで `PreviewPane` の timer を停止しない。preview 停止が必要なのは、GUI 設定変更で active capture device を切り替えるときである。

capture device 切り替え時は、`PreviewPane` の timer を停止し、古い device への参照を外してから新しい active device を設定し、表示中であれば同じ `preview_fps` で再開する。Runtime 実行中に capture ownership を排他する必要がある場合は、`FrameSourcePort` / capture adapter 側のロック契約として別途定義し、GUI preview 停止へ暗黙に押し込まない。
...
- capture device 設定変更時は preview timer を一時停止し、device 差し替え後に必要なら再開する。
...
- close timeout は `global_settings.get("runtime.release_timeout_sec", 5)` から読む。
...
- `MacroCatalog` の生成。
- global settings / secrets settings から runtime builder を構成し、snapshot 変更時だけ cached builder を差し替える。
- serial / capture / protocol / notification の変更反映。実行中に builder 再構成が必要な変更は即時反映しない。
...
`pause_for_device_switch()` は timer を停止する。`resume_after_device_switch()` は widget が表示中で、かつ capture device が設定済みの場合だけ現在の `preview_fps` で timer を再開する。Runtime 実行状態と snapshot 可否はこの API の責務に含めない。
...
| GUI | `test_preview_pauses_during_capture_device_switch` | capture device 切り替え中に preview timer を停止する |
| GUI | `test_preview_resumes_after_capture_device_switch_when_visible` | device 差し替え後、表示中なら preview を再開する |
...
- [ ] capture device 切り替え時の preview pause / resume を入れる。
...
| Capture switch gate | capture device 切り替え中に古い device 参照で preview 更新しないことをテストで防ぐ |
