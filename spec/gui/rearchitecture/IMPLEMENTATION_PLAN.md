...
> **対象モジュール**: `src\nyxpy\gui\`, `tests\gui\`
> **目的**: GUI の実行制御を `RunHandle` / `RunResult` / `UserEvent` ベースへ固定し、旧 `Command.stop()`、`MacroExecutor`、GUI スレッドからの Runtime 詳細操作、capture device 切り替え時の状態競合を排除する。
...
| 項目 | 現状 | 修正方針 |
|------|------|----------|
| Builder 生成 | 実行ごとに旧 Runtime builder 補助関数で生成する | `GuiAppServices.create_runtime_builder()` に集約し、起動時と設定変更時だけ再構成する。`legacy` 名の builder 補助関数は最終状態に残さない |
| Macro identity | `MacroCatalog.macros` が `definition.class_name` を key にしている | stable `definition.id` を実行 ID にし、表示名と分離する |
| capture device 切り替え | PreviewPane が古い active capture source を参照したままになり得る | active device の実変更時だけ `PreviewPane.pause()` / `set_frame_source()` / `resume()` で `FrameSourcePort` 差し替え中の更新を止める。設定画面を開いただけでは停止しない |
...
| close timeout | `wait(5)` が固定値 | `GuiAppServices` が保持する settings snapshot の `runtime.gui_close_wait_timeout_sec` から読み、未設定時は `5` 秒にする |
| singleton 直接操作 | `MainWindow` が manager 設定、release、settings 差分を直接扱う | `GuiAppServices` 相当の composition object へ寄せ、MainWindow は UI orchestration に絞る。新 GUI 経路では manager singleton を直接参照しない |
...
```text
MainWindow
  -> GuiAppServices(cached runtime_builder, registry, logging, device services / Port factories)
...
GUI は変更範囲が広く、composition root、macro identity、実行状態、preview device 切り替え、log sink、close cleanup が互いに絡む。本書は全体計画と完了ゲートの正本にし、実装詳細は次の phase 仕様へ分ける。
...
| Phase 3 | `PHASE_3_PREVIEW_AND_OBSERVABILITY.md` | preview の device 参照更新と `UserEvent` 表示を整理する | Phase 2 |
| Phase 4 | `PHASE_4_CLEANUP_AND_REGRESSION.md` | close cleanup、削除対象 API の回帰防止、最終テストを固定する | Phase 1-3 |
| Phase 5 | `PHASE_5_LEGACY_ROUTE_AND_FW_CLEANUP.md` | EventBus、仮想コントローラー旧入力、FW 残存旧経路の削除可否を整理する | Phase 1-4 |
...
| `spec\gui\rearchitecture\PHASE_3_PREVIEW_AND_OBSERVABILITY.md` | 新規 | preview device 切り替え、`GuiLogSink`、`LogPane` の詳細 |
| `spec\gui\rearchitecture\PHASE_4_CLEANUP_AND_REGRESSION.md` | 新規 | close cleanup、削除対象 API、最終回帰テストの詳細 |
| `spec\gui\rearchitecture\PHASE_5_LEGACY_ROUTE_AND_FW_CLEANUP.md` | 新規 | EventBus と FW 残存旧経路の棚卸し、削除条件、回帰テスト |
| `src\nyxpy\gui\main_window.py` | 変更 | 実行状態、builder lifetime、cleanup、error logging、preview device 切り替えを整理 |
...
| `src\nyxpy\gui\panes\preview_pane.py` | 変更 | capture device 切り替え時の停止、差し替え、再開を実装 |
...
| `src\nyxpy\gui\events.py` | 削除候補 | Phase 5 で EventBus 参照をなくせた場合に削除 |
| `src\nyxpy\gui\models\virtual_controller_model.py` | 変更候補 | Phase 5 で EventBus、serial device、protocol 直接保持を削除 |
...
| `tests\gui\test_preview_device_switch.py` | 新規 | capture device 切り替え時の preview 停止と再開を検証 |
...
class GuiAppServices:
    ...
    def apply_settings(self, settings_snapshot, secrets_snapshot) -> None: ...
    def close(self) -> None: ...
```
...
`create_runtime_builder()` は毎回新規 builder を組み立てない。起動時または `apply_settings()` で settings / secrets / device / protocol / notification の snapshot が変わったときにだけ再構成し、通常の実行開始では cached builder を返す。実行中に builder 再構成が必要な設定変更を受けた場合は、実行完了まで反映を保留するか、UI で変更適用不可にする。

### 3.3 Macro identity

マクロ実行に使う ID は `MacroDefinition.id` を正とする。class name は表示名または診断情報としてだけ扱い、実行 ID、選択 fallback、Resource scope には使わない。

| 値 | 用途 |
|----|------|
| `definition.id` | `RuntimeBuildRequest.macro_id`、ログ相関、Resource scope |
| `definition.display_name` | GUI 表示、dialog title |
| `definition.class_name` | 表示補助、診断情報。実行 ID としては使わない |
...
### 3.5 Preview と capture device 参照更新

Runtime 実行中という理由だけで `PreviewPane` の timer を停止しない。GUI 設定変更で active capture device が実際に変わる場合だけ、`PreviewPane.pause()` で更新を止め、`set_frame_source()` で GUI lifetime の `FrameSourcePort` を差し替え、`resume()` で通常の preview 更新へ戻す。設定画面を開いただけでは preview 状態を変更しない。

Runtime と preview の同時 capture 利用は `FrameSourcePort` / capture adapter 側のロック契約で扱い、GUI preview 停止へ暗黙に押し込まない。preview tick は `try_latest_frame()` で非ブロッキング取得し、source が busy の場合は描画を skip して直近 pixmap を維持する。
...
- capture device 設定変更で active device が実際に変わる場合だけ、`PreviewPane.pause()`、`set_frame_source()`、`resume()` の順で `FrameSourcePort` 参照を更新する。
...
- close timeout は `GuiAppServices` が保持する settings snapshot の `runtime.gui_close_wait_timeout_sec` から読む。
...
- `MacroCatalog` の生成。
- `SettingsStore` / `SecretsStore` 由来の snapshot から runtime builder を構成し、snapshot 変更時だけ cached builder を差し替える。
- serial / capture / protocol / notification の変更反映。実行中に builder または GUI lifetime Port の再構成が必要な変更は即時反映しない。
- application close 時の GUI lifetime Port / device service close と logging close。

GUI lifetime Port / device service close 例外は `LoggerPort.technical("WARNING", ..., event="resource.cleanup_failed")` へ記録する。`pass` で握りつぶさない。
...
最小実装では `set_run_state(state: RunUiState)` を追加する。既存 `set_running(bool)` は呼び出し元置換中の一時 wrapper に限り許可し、Phase 4 完了までに公開 API から削除する。最終状態の `ControlPane` 公開 API は `set_run_state()` を正とする。

### 4.5 `PreviewPane`

`PreviewPane` は汎用の停止 / 再開 API と frame source 差し替え API を提供する。
...
`pause()` は preview timer を停止し、`resume()` は widget が表示中で frame source が設定済みの場合だけ現在の `preview_fps` で timer を再開する。`set_frame_source()` は保持している `FrameSourcePort` 参照を差し替える。これらは device 切り替えにも使うが、API 名は特定用途に縛らない。Runtime 実行状態に連動した preview 停止、snapshot 可否の変更はこの仕様に含めない。`update_preview()` は UI thread から `FrameSourcePort.latest_frame()` を呼ばず、`try_latest_frame()` が `None` を返した tick は直近 pixmap を維持する。
...
`MainWindow.closeEvent()` は logging close より前に `LogPane` の sink を明示解除する。子 widget の `closeEvent()` が logging close 前に必ず呼ばれる前提にしない。実装は `LogPane.dispose()` のような冪等 method を追加し、`closeEvent()` からも同じ method を呼ぶ形にする。
...
| GUI | `test_main_window_close_logs_cleanup_failures` | GUI lifetime Port / device service close 失敗を沈黙させない |
| GUI | `test_preview_pauses_only_during_active_device_change` | 設定画面を開いただけでは停止せず、active device 実変更時だけ停止する |
| GUI | `test_preview_resumes_after_frame_source_update` | `FrameSourcePort` 参照差し替え後、表示中なら preview を再開する |
| GUI | `test_preview_skips_frame_when_source_busy` | `try_latest_frame()` が `None` の tick は直近 pixmap を維持する |
| 性能 | `test_preview_runtime_frame_source_contention_perf` | Runtime capture と preview tick の同時利用で preview tick 超過率が 1% 未満 |
...
| No legacy surface gate | `legacy` 名の builder 補助関数、`MacroCatalog.macros`、`ControlPane.set_running(bool)` が GUI の最終公開 API に残っていない |
