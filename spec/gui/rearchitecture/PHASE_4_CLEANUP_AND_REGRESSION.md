...
2. `global_settings.get("runtime.release_timeout_sec", 5)` から timeout 秒を読み、`wait(timeout)` する。
...
6. `GuiAppServices.close()` で manager release と logging close を行う。
7. `super().closeEvent(event)` を呼ぶ。

timeout は固定値ではなく `global_settings["runtime.release_timeout_sec"]` を正とする。設定がない場合の既定値は `5` 秒とする。`RuntimeOptions.release_timeout_sec` は Runtime 内部の port 解放 timeout であり、GUI close の `RunHandle.wait()` timeout として直接参照しない。
