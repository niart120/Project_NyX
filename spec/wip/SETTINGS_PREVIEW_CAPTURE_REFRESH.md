...
- 設定ダイアログを「デバイス・一般」「通知・ログ」の 2 タブ構成へ変更する。
...
| 新タブ | 旧タブ由来 | セクション順 |
|--------|------------|--------------|
| デバイス・一般 | デバイス + 一般の外観 | キャプチャ入力、シリアルデバイス、外観 |
| 通知・ログ | 通知 + 一般のログ | Discord 通知、Bluesky 通知、ログ |

`GeneralSettingsTab` は最終状態では残さない。外観セクションは新しいデバイス・一般タブへ移動し、ログセクションは通知・ログタブへ移動する。既存 class を分割移動の一時経路として使う場合も、公開される最終 UI は 2 タブ構成にする。

設定ダイアログのタイトルは `デバイス・通知・ログ設定` に変更する。
...
|--------|--------------|------------------|
| `camera` | Source、Camera、Capture FPS、Aspect Box、Preview FPS | Window、Window Match、Backend |
| `window` | Source、Window、Window Match、Backend、Capture FPS、Aspect Box、Preview FPS | Camera |
...
`ControllerOutputPortFactory` は `serial_device` を display name ではなく identifier として解釈する。検出結果から `identifier` が一致する `DeviceInfo` を探し、`SerialComm(str(info.identifier))` で接続する。未検出時のエラー表示には display name と identifier の両方を含める。
...
| `src\nyxpy\gui\dialogs\settings\device_tab.py` | 外観セクション統合、`screen_region` / Region 削除、Source 別表示切替、serial combo の item data 化 |
...
| `src\nyxpy\framework\core\hardware\capture_source.py` | `ScreenRegionCaptureSourceConfig` と region parsing を削除 |
| `src\nyxpy\framework\core\hardware\window_capture.py` | auto backend fallback を追加。未使用になる `ScreenRegionCaptureDevice` の扱いを整理 |
...
| `test_settings_tabs_are_device_general_and_notification_log` | 設定タブが 2 個で、順に「デバイス・一般」「通知・ログ」である |
