...
> **関連ドキュメント**: `FOLLOWUP_FIXES.md`, `spec\framework\rearchitecture\IMPLEMENTATION_PLAN.md`, `spec\framework\rearchitecture\RUNTIME_AND_IO_PORTS.md`, `spec\framework\rearchitecture\LOGGING_FRAMEWORK.md`, `spec\framework\rearchitecture\OBSERVABILITY_AND_GUI_CLI.md`, `spec\framework\rearchitecture\DEPRECATION_AND_MIGRATION.md`
...
| 項目 | 現状 | 修正方針 |
|------|------|----------|
| Builder 生成 | 旧 Runtime builder 補助関数を CLI から呼ぶ | CLI は `MacroRuntimeBuilder` interface だけに依存する。`legacy` 名の補助関数は最終状態に残さない |
| helper docstring | `create_runtime_builder()` が Command 生成と説明している | Runtime builder 生成の説明へ修正する |
| cleanup | `finally` で resource close 例外を `pass` している | 例外は `LoggerPort.technical()` へ記録し、終了コードの主結果は上書きしない |
| device 選択 | CLI は manager の active device を直接取得しない | 検出、dummy 許可、未選択エラーは builder の責務に固定する |
| 通知 secret | secrets snapshot から通知 adapter を生成している | CLI 引数、`exec_args`、通常 settings へ secret を渡さないことをテストで固定する |
...
run_cli.main()
  ...
  -> close logging and composition-root resources
```
...
| `spec\cli\rearchitecture\FOLLOWUP_FIXES.md` | 新規 | CLI 追加修正の詳細 |
...
| 依存生成 | logger、settings / secrets snapshot、device discovery、Port factory、registry、builder を組み立てる | `DefaultCommand` を構築する、manager singleton から active device を取る |
...
通知 secret を受け取る `--discord-webhook-url`、`--bluesky-password` などの CLI 引数は追加しない。通知設定は `SecretsStore` 由来の secrets snapshot だけを入力元とする。
...
CLI は builder に device 名、baudrate、protocol、logger、settings / secrets snapshot、device discovery、Port factory を渡す。通知は builder が secrets snapshot から `NotificationPort` を構築する。実デバイスの登録待ち、dummy device の許可判定、未選択エラー、frame readiness は builder / Port adapter の責務である。

CLI から `serial_manager.get_active_device()`、`capture_manager.get_active_device()`、`DefaultCommand(...)` を呼ばない。新 CLI 経路では manager singleton を直接 release せず、`MacroRuntimeBuilder.shutdown()` と logging close で composition-root resource を閉じる。
...
`create_runtime_builder()` は CLI 内部 helper として扱う。既存テストから import されてもよいが、外部互換 API として保証しない。最終状態では `legacy` 名の Runtime builder 補助関数へ依存しない。

必須要件:

- docstring は Runtime builder 生成を説明する。
- `SecretsStore` 由来の secrets snapshot を読み、builder へ渡す。
...
`finally` では `MacroRuntimeBuilder.shutdown()` と logging close を行う。close 例外は logger が生きている場合に `technical("WARNING", ..., event="resource.cleanup_failed")` へ記録する。logger が生成されていない段階の cleanup 失敗は `stderr` へ短文を出す。
...
| Phase 3: error mapping / cleanup 可視化 | 構成不正と cleanup 失敗を沈黙させない | `ConfigurationError` の終了コード `1` 化、builder shutdown / logging close 失敗 logging | cleanup 失敗が `resource.cleanup_failed` として残る |
...
| 結合 | `test_cli_notification_settings_source_is_secrets_store` | 通知設定の入力が `SecretsStore` 由来の secrets snapshot である |
| 結合 | `test_cli_cleanup_failures_are_logged` | builder shutdown / logging close 例外を沈黙させない |
...
- [ ] `legacy` 名の Runtime builder 補助関数への依存を削除する。
...
| Secret source gate | 通知 secret は `SecretsStore` 由来の secrets snapshot だけから生成する |
| Exit code gate | `RunResult` と構成不正から終了コードを一意に決める |
| Cleanup visibility gate | builder shutdown / logging close 失敗を沈黙させない |
| Removal gate | `MacroExecutor`、`DefaultCommand`、`LogManager`、`log_manager` 参照が CLI にない |
| No legacy helper gate | `legacy` 名の Runtime builder 補助関数が CLI に残っていない |
