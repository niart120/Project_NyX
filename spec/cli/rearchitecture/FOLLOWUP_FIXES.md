...
- [ ] `legacy` 名の Runtime builder 補助関数依存を削除する。
- [ ] CLI の secret 入力元を `SecretsStore` snapshot に統一する。
- [ ] manager singleton の active device 取得と release を CLI から削除する。
- [ ] `MacroRuntimeBuilder.shutdown()` の cleanup 失敗を技術ログまたは stderr へ残す。
- [ ] CLI parser に通知 secret 引数がないことを固定する。
- [ ] 静的 import / call graph テストで旧 API への逆戻りを防ぐ。
