...
### 1.1 本書の目的
本書は、Nintendo Switch 向け自動化支援フレームワーク「Project NyX」の設計思想、全体アーキテクチャ、およびコンポーネント間の連携について説明するものです。旧 `docs` から移設したアーカイブであり、現行コードでは `src/nyxpy/` 配下の実装が正本です。
...
- Managerインスタンスは `singletons.py` で一元管理され、GUI は Manager、`EventBus`、`VirtualControllerModel` を組み合わせて参照・切替を行う

### 2.3 通信プロトコル層
高レベル操作命令をデバイス固有のバイナリプロトコルに変換します。

- `SerialProtocolInterface` による抽象化
- プロトコル実装（`CH552SerialProtocol`, `PokeConSerialProtocol`, `ThreeDSSerialProtocol`）は交換可能
- `ProtocolFactory` がプロトコル名、別名、既定ボーレート、対応ボーレートを管理
- プロトコル仕様の詳細は別文書に記載
...
- `reload_macros()`: `macros` ディレクトリから利用可能なマクロをロード
- `set_active_macro(macro_name)`: 指定マクロをアクティブに設定
...
- `type(key)`: 個別キー入力
- `notify(text, img)`: 外部通知
- `touch(x, y, dur, wait)`, `touch_down(x, y)`, `touch_up()`: 3DS 向けタッチ入力
- `disable_sleep(enabled)`: 3DS 向けスリープ抑止
...
class DefaultCommand(Command):
    def __init__(self, serial_device, capture_device, resource_io, protocol, ct, notification_handler):
...
#### 3.3.1 シングルトンと GUI 状態管理
ハードウェア操作の依存性を一元管理するため、Managerインスタンスは `singletons.py` でグローバルに生成されます。GUI は `MainWindow`、各 Pane、`EventBus`、`VirtualControllerModel` から Manager を参照します。

- `EventBus` がシリアルデバイス変更、キャプチャデバイス変更、プロトコル変更を通知
- `VirtualControllerModel` が仮想コントローラーの状態管理とシリアル送信を担当
- 各GUIコンポーネントやコマンドは Manager からデバイスを受け取って操作
...
class SerialProtocolInterface(ABC):
    ...
    def build_keytype_command(self, key: KeyCode | SpecialKeyCode, op: KeyboardOp) -> bytes:
        """キーボード個別キー操作のコマンドデータを生成"""
        pass
```

#### 3.4.2 CH552SerialProtocol
CH552デバイス向けの具体的なプロトコル実装です。
詳細仕様は `./protocol/ch552_protocol_spec.md` に記載されています。
...
1. **マクロロード**:
   ...
   - `set_active_macro(name)` で実行対象マクロを設定
...
- **マクロの再開機能**:
  中断は `CancellationToken` で実装済み。再開は未実装
...
- **ThreeDSSerialProtocol**: Nintendo 3DS 向け S2/T3 系プロトコル
...
- Manager系（SerialManager, CaptureManager）とSettings系（GlobalSettings, SecretsSettings）は`singletons.py`で一元管理
