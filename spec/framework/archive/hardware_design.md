...
このドキュメントでは、Nintendo Switch との物理的連携部分、特にキャプチャボードおよびシリアル通信に関する設計方針と実装概要を記載します。旧 `docs` から移設したアーカイブであり、現行コードでは `src/nyxpy/framework/core/hardware/` が正本です。
...
- **主なメソッド:**  
  ...
  - `get_frame()`: キャッシュされた最新フレームをスレッドセーフに返す  
...
- **SerialCommInterface:**  
  - シリアル通信に必要な基本操作（`open`, `send`, `close`）を定義
- **SerialComm:**  
  - pyserial を利用した実装例
...
### SerialProtocolInterface と ProtocolFactory
- **目的:**  
  - 高レベル操作（press, hold, release, keyboard, type など）を、通信プロトコルに基づいたコマンドデータに変換する
- **主なメソッド:**
  - `build_press_command(keys)`: キー押下コマンドの生成
  - `build_hold_command(keys)`: 現在の入力状態を指定キー集合へ置き換えるコマンドの生成
  - `build_release_command(keys)`: キー解放コマンドの生成
  - `build_keyboard_command(text)`: 文字列入力コマンドの生成
  - `build_keytype_command(key, op)`: 個別キー入力コマンドの生成
- **設計ポイント:**
  - `CH552`, `PokeCon`, `3DS` は `ProtocolFactory` で選択される。
  - `DefaultCommand` から呼び出され、GUI/CLI/マクロはプロトコル固有のバイト列を意識しない。
...
- **依存性注入:**  
  - `DefaultCommand` は `SerialCommInterface`、`CaptureDeviceInterface`、`StaticResourceIO`、`SerialProtocolInterface`、`CancellationToken`、`NotificationHandler` を外部から受け取る。
