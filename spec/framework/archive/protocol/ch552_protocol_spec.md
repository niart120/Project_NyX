...
キーボードの個別キー入力は `kbdheader`, `key` の2つのフィールドで管理されます。現行の `CH552SerialProtocol.build_keyboard_command(text)` は文字列一括入力に対応せず、`NotImplementedError` を送出します。`DefaultCommand.keyboard(text)` はこの例外を受け、文字ごとの `type()` にフォールバックします。

#### **build_keytype_command**
- `kbdheader`: `KeyboardOp` の値
  - `0x01`: 通常キー押下
  - `0x02`: 通常キー解放
  - `0x03`: 特殊キー押下
  - `0x04`: 特殊キー解放
  - `0x05`: 全キー解放
- `key`: 入力するキーの ASCII コードまたは特殊キーコード
