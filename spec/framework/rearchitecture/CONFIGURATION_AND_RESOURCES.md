...
絶対パス、空 path、`..` による root 外参照、解決後に許可 root 外へ出るシンボリックリンクは `ConfigurationError` とする。

`MacroSettingsResolver.resolve(definition)` は、有効な探索候補が存在する場合に `MacroSettingsSource` を返す。manifest 指定、legacy project root、`cwd` fallback のいずれも存在しない場合は `None` を返す。不正 path、許可 root 外参照、root 外シンボリックリンク、manifest で required flag を導入した場合の未存在は `ConfigurationError` とする。
