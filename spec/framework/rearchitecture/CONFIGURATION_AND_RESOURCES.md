...
再設計では、settings TOML の探索、TOML parse、schema 検証、secret 境界を本書で扱う。`cmd.load_img()`、`cmd.load_blob()`、`cmd.save_artifact_*()`、assets / artifacts 配置、path traversal 防止、atomic write は `RESOURCE_FILE_IO.md` を正とする。
...
`MacroSettingsResolver` は settings TOML の探索と読み込みだけを公開 API とする。`Command.load_img()` / `Command.load_blob()` / `Command.save_artifact_*()` は `RESOURCE_FILE_IO.md` の API とし、本書では settings lookup と混同しないことだけを契約にする。
...
`MacroSettingsResolver.load(definition)` は `resolve()` が `None` を返した場合に `{}` を返す。明示 settings path の解決先が存在しない場合は `ConfigurationError(code="NYX_SETTINGS_NOT_FOUND")`、OS エラーで読めない場合は `ConfigurationError(code="NYX_SETTINGS_READ_FAILED")`、settings TOML の parse 失敗は `ConfigurationError(code="NYX_SETTINGS_PARSE_FAILED")`、schema 型不一致と secret 境界違反は `ConfigurationError` とし、既定値へ fallback しない。fallback すると実行引数の誤りを成功扱いにするためである。エラー message と `details` には `macro_id`、元の `settings_path`、解決後の `resolved_path`、`source` を含める。Runtime builder は実行を開始せず、`LoggerPort.technical()` は `LOGGING_FRAMEWORK.md` の規則に従って `error_code` と `error_details` を `TechnicalLog.extra` に残す。
...
`Command.load_img()` / `load_blob()` / `save_artifact_*()` の仕様、`resources\<macro_id>\assets` と `resources\<macro_id>\artifacts` の標準配置、path traversal 防止、atomic write は `RESOURCE_FILE_IO.md` を参照する。
...
| `ConfigurationError(code="NYX_SETTINGS_NOT_FOUND")` | 明示 settings path の解決先ファイルが存在しない |
| `ConfigurationError(code="NYX_SETTINGS_READ_FAILED")` | 明示 settings path の解決先ファイルを OS エラーで読み込めない |

`ConfigurationError.code` は `ERROR_CANCELLATION_LOGGING.md` の Error code catalog を正とする。本書では `NYX_SETTINGS_NOT_FOUND`、`NYX_SETTINGS_READ_FAILED`、`NYX_SETTINGS_PARSE_FAILED`、`NYX_SETTINGS_SCHEMA_INVALID`、`NYX_SETTINGS_PATH_INVALID`、`NYX_DEFINE_PARSE_FAILED`、`NYX_DEFINE_INVALID`、`NYX_MACRO_ARGS_INVALID` を参照し、別名を定義しない。
...
| ユニット | `test_missing_resource_settings_path_reports_macro_context` | 明示 `resource:` settings のファイル欠落時に `NYX_SETTINGS_NOT_FOUND` と `macro_id` / `resolved_path` を含む詳細を返す |
...
Resource File I/O の path guard、画像・bytes 読み書き、atomic write、`cmd.load_img()` / `load_blob()` / `save_artifact_*()` テストは `RESOURCE_FILE_IO.md` のテスト方針を正とする。
