...
> **破壊的変更**: settings lookup は破壊的変更を許容する。`static\<macro_name>\settings.toml` と `Path.cwd()` fallback は互換維持対象に含めず、manifest または class metadata の明示 settings source と明示 `project_root` へ移行する。
...
schema は dotted key を受け付け、TOML 読み込み後の辞書に対して既定値適用と型検証を行う。`SettingsStore` は secret flag を持つ field を禁止する。`SecretsStore` は通知 secret を保持し、Runtime builder は通知 adapter へ secret 値を渡す直前だけ平文を取り出す。
...
`MacroSettingsResolver.resolve(definition)` は、有効な明示 settings path が存在する場合に `MacroSettingsSource` を返す。manifest / class metadata 指定が存在しない場合は `None` を返す。不正 path、許可 root 外参照、root 外シンボリックリンク、required flag を導入した場合の未存在は `ConfigurationError` とする。
...
| `SecretBoundaryError` | 通常設定へ secret field を定義、または Runtime builder が secrets snapshot 以外から secret 値を受け取った |
