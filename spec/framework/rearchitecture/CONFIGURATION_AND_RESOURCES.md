...
> **破壊的変更**: settings lookup は破壊的変更を許容する。`static\<macro_name>\settings.toml` と `Path.cwd()` fallback は互換維持対象に含めず、manifest settings path と明示 `project_root` へ移行する。
...
| MacroSettingsResolver | `macro.toml` の settings 指定を解決し、マクロ実行引数へ渡す辞書を作るコンポーネント |
...
現行 `load_macro_settings()` は `Path.cwd()\static\<macro_name>\settings.toml` を暗黙に読む。画像入出力も `static` 配下にあるため、settings lookup と resource file I/O の境界が曖昧になっていた。再設計ではこの暗黙探索を残さず、manifest settings path と明示 `project_root` へ移行する。
...
| `static\<macro_name>\settings.toml` 互換 | 暗黙維持 | 削除し、manifest settings path と移行ガイドで置き換える |
...
- `static\<macro_name>\settings.toml` は標準探索先にしない。必要な settings は manifest settings path へ移行する。
...
| `src\nyxpy\framework\core\macro\settings_resolver.py` | 新規 | `MacroSettingsResolver` と manifest settings 解決を実装 |
...
| `tests\unit\macro\test_settings_resolver.py` | 新規 | manifest settings の解決と旧 fallback 不使用を確認 |
...
`static\<macro_name>\settings.toml` は互換契約に含めない。`macro.toml` が settings path を明示した場合だけ macro settings file を読み込む。明示がない場合は `resolve()` が `None`、`load()` が `{}` を返す。`Path.cwd()` 由来の fallback は残さない。
...
| macro settings | manifest settings の解決と辞書化 | settings schema、`MacroDefinition` |
...
| `MacroSettingsResolver.resolve()` | manifest settings path で 10 ms 未満 |
...
| `macro.toml [macro].settings` | `str | None` | `None` | manifest で明示する macro settings path。未指定なら macro settings file は読み込まない |
...
| 3 | manifest 指定なし | `None`。macro settings file は読み込まない |
...
`MacroSettingsResolver.resolve(definition)` は、有効な manifest settings path が存在する場合に `MacroSettingsSource` を返す。manifest 指定が存在しない場合は `None` を返す。不正 path、許可 root 外参照、root 外シンボリックリンク、manifest で required flag を導入した場合の未存在は `ConfigurationError` とする。
...
`ConfigurationError.code` は `ERROR_CANCELLATION_LOGGING.md` の Error code catalog を正とする。本書では `NYX_SETTINGS_PARSE_FAILED`、`NYX_SETTINGS_SCHEMA_INVALID`、`NYX_DEFINE_PARSE_FAILED`、`NYX_DEFINE_INVALID`、`NYX_MACRO_ARGS_INVALID` を参照し、別名を定義しない。
...
| 結合 | `test_migrated_macro_settings_load_from_manifest` | 移行後マクロの settings が manifest settings path から読み込まれる |
