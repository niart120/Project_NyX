...
> **破壊的変更**: なし。図は仕様理解を補助する文書であり、既存マクロの import contract を変更しない。
...
| 用語 | 定義 |
|------|------|
| Import Contract | 既存マクロが import している `MacroBase`、`Command`、`DefaultCommand`、constants、`MacroStopException`、settings lookup を維持する契約。 |
| Legacy Compatibility Layer | import contract と旧入口を守る互換層。`MacroExecutor` は必要な場合だけ一時 adapter として扱う。 |
| MacroRuntime | マクロ実行要求、registry 解決、factory 呼び出し、context 生成、runner 呼び出し、結果返却を統括する中核。 |
| MacroRegistry | マクロ定義、安定 ID、ロード診断を保持するレジストリ。実行インスタンスは保持しない。 |
| MacroFactory | `MacroRegistry` の定義から実行ごとに新しい `MacroBase` インスタンスを生成するコンポーネント。 |
...
| CommandFacade | 既存 `Command` API を維持し、実処理を Ports へ委譲する実装。 |
...
- `RUNTIME_AND_IO_PORTS.md` の Port 名、Adapter 名、CommandFacade 方針を前提にする。
- `MACRO_COMPATIBILITY_AND_REGISTRY.md` の Compatibility Contract を破壊不可境界として扱う。
- `OBSERVABILITY_AND_GUI_CLI.md` の GUI/CLI 入口、LogManager、通知設定ソースの方針を反映する。
...
本仕様は再設計仕様群の索引図である。詳細な API、例外、設定値、テスト項目は既存の各仕様を正とし、本仕様は依存方向と境界の読み違いを防ぐための補助仕様として扱う。
...
既存マクロは `MacroRuntime`、`MacroRegistry`、Ports を直接使う必要がない。マクロ作者に要求する互換条件は現行と同じ import path、`MacroBase.initialize(cmd, args)` / `run(cmd)` / `finalize(cmd)`、`Command` 公開メソッド、`static/<macro_name>/settings.toml` 参照である。

### レイヤー構成

- GUI/CLI は Runtime を呼ぶ上位 adapter である。
- 既存マクロは import contract へ依存する利用者コードである。
- Runtime 中核は registry、factory、context、runner、result を統括する。
- `CommandFacade` は既存 `Command` API を維持し、I/O は Ports へ委譲する。
...
flowchart TB
    ...
    subgraph Compat["Legacy Compatibility Layer<br/>破壊不可 import contract"]
        ...
        SettingsNode["static/&lt;macro_name&gt;/settings.toml lookup"]
        ExecutorNode["MacroExecutor<br/>残す場合だけ一時 adapter"]
    end
    ...
    subgraph RuntimeCore["Framework Runtime core"]
        ...
        CommandFacade["CommandFacade implements Command"]
    end
    ...
    Runtime --> Runner
    Runtime --> Result
    Runtime --> CommandFacade
    Registry -. "動的ロードのみ" .-> MacroCode
    Factory -->|new instance per run| MacroCode
    Runner -->|initialize / run / finalize| MacroCode
    Runner --> CommandNode
    CommandNode --> CommandFacade

    CommandFacade --> ControllerPort
    CommandFacade --> FramePort
    CommandFacade --> ResourcePort
    CommandFacade --> NotificationPortNode
    CommandFacade --> LoggerPortNode
    ...
    class Runtime,Registry,Factory,Context,Runner,Result,CommandFacade runtime;
...
実行要求が registry、factory、context、runner、result へ流れる順序を示す。`MacroExecutor` が残る場合も、同じ Runtime 経路へ委譲する。

```mermaid
flowchart LR
    Request["request<br/>GUI / CLI / legacy adapter"]
    RegistryStep["registry<br/>MacroRegistry.resolve"]
    FactoryStep["factory<br/>MacroFactory.create"]
    ...
    ContextStep -->|CommandFacade(context)| RunnerStep
...
`CommandFacade` から controller、capture、resource、notification、logger の各 Port へ分岐し、Adapter が現行実装または外部 I/O へ接続する。

```mermaid
flowchart TB
    CommandFacade["CommandFacade<br/>既存 Command API"]
    ...
    subgraph ConcreteIO["Concrete I/O"]
        ...
        StaticRoot["static root<br/>画像リソース"]
        SettingsResolver["MacroSettingsResolver<br/>settings TOML 解決"]
        Discord["Discord"]
        Bluesky["Bluesky"]
        LogManager["LogManager<br/>構造化ログ / GuiLogEvent"]
    end

    CommandFacade --> Controller
    CommandFacade --> Capture
    CommandFacade --> Resource
    CommandFacade --> Notify
    CommandFacade --> Logger
    CommandFacade -. "touch*" .-> Touch
    CommandFacade -. "disable_sleep" .-> Sleep
    ...
    Logger --> LogManagerPort --> LogManager
    ...
    class CommandFacade facade;
    class Controller,Capture,Resource,Notify,Logger,Touch,Sleep port;
    class SerialPort,DummyController,CapturePort,DummyFrame,StaticStore,NotificationHandlerPort,LogManagerPort adapter;
    class SerialComm,Protocol,CaptureDevice,StaticRoot,SettingsResolver,Discord,Bluesky,LogManager io;
```

### 4.4 互換境界図

既存ユーザーマクロが依存してよい領域、内部差し替え可能な領域、一時 adapter / 廃止候補領域を分ける。
...
flowchart TB
    subgraph User["User macro import contract"]
        ...
        StaticSettings["static/&lt;macro_name&gt;/settings.toml"]
    end
    ...
    subgraph Stable["破壊不可領域"]
        ...
        SettingsContract["legacy settings lookup"]
    end
    ...
    subgraph Deprecated["一時 adapter / 廃止候補領域"]
        Executor["MacroExecutor<br/>旧 GUI/CLI/テスト入口"]
        LegacyLoader["legacy loader"]
    ...
    UserCode --> StaticSettings --> SettingsContract

    Runner --> MacroBaseContract
    Facade --> CommandContract
    Registry --> SettingsContract
    Runtime --> Registry
    Runtime --> Factory
    Runtime --> Runner
    Facade --> Ports

    Executor -. "残す場合は委譲" .-> Runtime
    LegacyLoader -. "Registry へ統合" .-> Registry
    CwdFallback -. "非推奨警告付き fallback" .-> Registry
    ...
    class Runtime,Registry,Factory,Runner,Facade,Ports replaceable;
...
flowchart LR
    ...
    LogManager --> TechLog["技術ログ<br/>traceback / masked extra"]
    LogManager --> GuiEvent["GuiLogEvent<br/>短い表示文言"]
    Result --> CliPresenter["CliPresenter<br/>exit code / UserMessage"]
    Result --> GuiPresenter["GUI presenter<br/>status / dialog"]
    Handle --> GuiPresenter
    GuiEvent --> GuiPresenter
    ...
    class Logger,LogManager,TechLog,GuiEvent obs;
```
...
本仕様はシングルトンを追加しない。`MacroRuntime`、`MacroRegistry`、`LogManager`、既存 manager の singleton 管理方針は各詳細仕様を正とする。
