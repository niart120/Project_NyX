...
| Compatibility Layer | import contract を守る互換層。`MacroExecutor`、旧 settings lookup、旧 Resource I/O は含めず、GUI/CLI/テストは新 Runtime 入口へ移行済みである。 |
...
flowchart TB
    ...
    subgraph Removed["Removed legacy APIs<br/>互換契約外"]
        ExecutorNode["MacroExecutor<br/>module removed"]
    end
    ...
    subgraph Adapters["Adapters"]
        ...
        ResourceStoreAdapter["LocalResourceStore"]
        ArtifactAdapter["LocalRunArtifactStore"]
        NotificationAdapter["NotificationHandlerPort"]
        LoggerAdapter["DefaultLogger / GuiLogSink"]
    end
    ...
    classDef removed fill:#ffebee,stroke:#c62828,stroke-width:1px;
...
| removed | 削除済みで import shim を持たない旧実装 |
| guard | 静的依存禁止の境界 |
...
flowchart TB
    ...
    subgraph Deprecated["削除済み / 廃止候補領域"]
        Executor["MacroExecutor<br/>module removed"]
    ...
    Executor -. "import shim なし" .-> Runtime
    ...
    class UserCode,ImportBase,ImportCommand,ImportConstants,ImportStop,ManifestSettings user;
    class MacroBaseContract,CommandContract,ConstantsContract,StopContract,SettingsResolverContract stable;
