...
> **目的**: フレームワーク再設計を安全に進めるため、維持する公開互換、マクロ移行後の受け入れ、Runtime 分割、Port adapter、GUI/CLI、実機、性能、スレッド安全性、キャンセル応答の検証方針を定義する。
...
> **破壊的変更**: 破壊的変更と削除条件は `DEPRECATION_AND_MIGRATION.md` を正とする。本書はそれらを検出するテスト配置、テスト名、ゲート条件を定義する。

## 1. 概要

### 1.1 目的

フレームワーク内部を抜本改修しても、維持対象の import / lifecycle 互換と移行後マクロの動作が壊れないことを、テストで先に固定する。Registry、Factory、Runner、Runtime、Ports、GUI/CLI adapter を段階別に検証し、実機依存テストは `@pytest.mark.realdevice` で通常テストから分離する。
...
| MacroRuntime | Registry / `MacroDefinition.factory` / Runner / Ports を統合して同期・非同期実行を提供する中核 |
...
| キャンセル応答 | 体感確認になりやすい | 100 ms 未満の目標をテストで測定 |
| スレッド安全性 | handler 例外や lock 競合が未検出 | 並行テストで deadlock と競合を検出 |

### 1.5 着手条件

- 維持対象 import / lifecycle の互換確認では既存マクロ本体を編集しない。repository macro の移行テストは移行後の manifest / settings / resources 配置を対象にする。
- 代表マクロの import、entrypoint、settings、lifecycle を受け入れ対象として選定する。
- 実機不要の通常テストは fake adapter と `tmp_path` を使い、リポジトリの `macros\` と `resources\` を破壊しない。
...
| `tests\unit\framework\macro\test_import_contract.py` | 新規 | import path と公開シグネチャの互換を検証 |
| `tests\fixtures\macros\manifest_package\macro.py` | 新規 | manifest entrypoint を持つ package 形式マクロ fixture |
| `tests\fixtures\macros\convention_package\macro.py` | 新規 | manifest なし convention discovery の package 形式マクロ fixture |
| `tests\fixtures\macros\convention_single_file.py` | 新規 | manifest なし convention discovery の single-file マクロ fixture |
| `tests\fixtures\resources\manifest_package\settings.toml` | 新規 | 明示 settings source fixture |
| `tests\unit\framework\macro\test_registry.py` | 新規 | `MacroRegistry` と `MacroDefinition.factory` の単体テスト |
| `tests\unit\framework\runtime\test_runner.py` | 新規 | `MacroRunner` の lifecycle、例外、中断、finalize を検証 |
| `tests\unit\framework\runtime\test_runtime.py` | 新規 | `MacroRuntime` と `RunHandle` の単体テスト |
| `tests\unit\framework\io\test_fake_ports.py` | 新規 | Port 契約を fake adapter で検証 |
| `tests\integration\test_migrated_macro_compat.py` | 新規 | 移行後マクロ fixture と repository macro の互換検証 |
...
| 互換対象 | constants import、`MacroStopException` | import、例外捕捉で固定 |
| 互換対象外 | 旧 settings lookup、旧 auto discovery、`DefaultCommand` 旧コンストラクタ、`MacroExecutor`、GUI/CLI 内部入口、singleton 直接利用、暗黙 dummy fallback | 削除確認テストと新 Runtime 入口テストで固定。軽量マクロは convention discovery で扱う |
...
`MacroExecutor` は公開互換契約から明示的に除外する。テストは成功時 `None`、失敗時例外再送出、`macros` / `macro` 属性を保証しない。`test_macro_executor_removed` は `nyxpy.framework.core.macro.executor` の import が `ModuleNotFoundError` になることを期待する。`test_gui_cli_entrypoints_do_not_import_macro_executor` は GUI/CLI entrypoint の import graph に `MacroExecutor` が含まれないことを確認する。
...
| `definition.factory.create()` | 1 回 5 ms 未満 |
| `Command.wait()` の `safe_point_latency` | token 発火後 100 ms 以内 |
...
| 判定値 | P95 をしきい値と比較する。`sorted(samples)[ceil(n * 0.95) - 1]` を採用し、単発上振れでは失敗させない |
...
P95 計算では `n < 10` のサンプル列を性能テストとして扱わない。実装上の都合で 10 回未満しか測れない処理は `tests\hardware\` または記録用テストへ分離し、CI の fail 条件にしない。

性能しきい値は初回実測後に仕様書とテスト定数を同じ変更で更新する。更新責任は対象コンポーネントの実装 PR が持ち、`TEST_STRATEGY.md` の目標値、`tests\perf\` の期待値、関連仕様の性能要件を同一 PR で揃える。CI で恒常的に 20% 許容幅を超える場合は、しきい値を緩める前に測定対象外コストが混入していないことを確認する。

| 測定対象 | 開始点 | 終了点 | 配置 | fail 条件 |
|----------|--------|--------|------|-----------|
| `safe_point_latency` | token 発火 | `Command.wait()` が `MacroCancelled` または `result.status == RunStatus.CANCELLED` へ到達 | `tests\perf\` | P95 が 100 ms 超 |
| `cancel_result_latency` | `RunHandle.cancel()` 呼び出し直前 | `handle.result().status == RunStatus.CANCELLED` | `tests\perf\` | P95 が 100 ms 超。ただし macro が safe point 外にいるケースは別テスト |
| `cancel_request_latency` | GUI cancel button click | `CancellationToken.request_cancel()` 呼び出し | `tests\gui\` / `tests\perf\` | P95 が 100 ms 超 |
...
`LogSinkDispatcher` は sink snapshot を lock 外で呼ぶため、sink が登録解除を行っても deadlock しないことを検証する。settings は snapshot を実行 context へ渡し、実行中の設定変更が進行中 context を破壊しないことを検証する。
...
class FakeControllerOutputPort:
    ...
    def type_key(self, key: KeyboardKey) -> None: ...
    def close(self) -> None: ...
...
| `test.safe_point_latency_limit_sec` | `float` | `0.1` | token 発火から safe point で中断例外になるまでの上限秒数 |
| `test.cancel_request_latency_limit_sec` | `float` | `0.1` | GUI cancel から token 発火までの上限秒数 |
| `test.cancel_result_latency_limit_sec` | `float` | `0.1` | `RunHandle.cancel()` から `result.status == RunStatus.CANCELLED` までの上限秒数 |
...
#### DefaultCommand / ExecutionContext fixture

`DefaultCommand` 旧コンストラクタ削除後の単体テストは `tests\support\fake_execution_context.py` の fixture を正本として使う。GUI/CLI/perf/結合テストは `DefaultCommand(serial_device=..., capture_device=..., ...)` を直接生成せず、fake `ExecutionContext` または `MacroRuntimeBuilder` 経由の context を渡す。

| fixture | 配置 | 用途 |
|---------|------|------|
| `fake_execution_context()` | `tests\support\fake_execution_context.py` | fake Ports、fake logger、fake resource store を束ね、`DefaultCommand(context=...)` の単体テストに使う |
| `fake_runtime_build_request()` | `tests\support\fake_execution_context.py` | builder 経由の GUI/CLI adapter テストで使う |

#### import/signature 互換テスト

`inspect.signature()` で既存公開 API の引数名、既定値、戻り値注釈を検証する。抽象クラスの内部実装は問わず、既存マクロが import して呼べる表面を固定する。互換対象の `Command` メソッドでは、`None` から `ResourceRef` や `int` へ戻り値注釈を変える変更も破壊的変更として扱う。新規 API と互換対象外 API はこの固定対象から除外する。

#### 既存マクロ fixture による互換検証

fixture は package 形式、single-file 形式、manifest entrypoint、convention discovery、class metadata、壊れた macro、settings あり、settings なしを含める。repository macro 互換テストは移行後の代表マクロをロードし、`MacroDefinition` 生成、settings 解決、`initialize -> run -> finalize` の最小 dry-run を検証する。

#### Registry / Factory / Runner / Runtime

Registry はロード診断と alias 解決を単体で検証する。`MacroDefinition.factory` は毎回新しいインスタンスを返すことを検証する。Runner は fake `Command` を使い、成功、例外、中断、finalize 失敗、`finalize(cmd, outcome)` opt-in を検証する。Runtime は fake Ports で context 生成、同期実行、非同期実行、Port close、`cleanup_warnings` を検証する。
...
CLI は subprocess 相当または `main(args)` 直接呼び出しで、Runtime builder 使用、`RunResult` 由来の終了コード、secrets snapshot 由来の通知設定を検証する。GUI は pytest-qt を使い、実行開始で `RunHandle` を保持し、cancel ボタンで `RunHandle.cancel()` を呼び、`UserEvent` を `LogPane` に表示することを検証する。

| テスト粒度 | 対象 | 検証内容 |
|------------|------|----------|
| GUI sink unit | `GuiLogSink` | `LogSink` Protocol 実装と Qt Signal emit。core 層の import を含めない |
| GUI adapter integration | `MainWindow` / worker adapter | 実行開始で `RunHandle` を保持し、cancel で `RunHandle.cancel()` だけを呼ぶ |
| GUI 表示 integration | `LogPane` | pytest-qt で queued connection 後の表示整形と行追加を検証する |
| CLI integration | `run_cli.main(args)` | Runtime builder 使用、`RunResult` 由来の終了コード、secrets snapshot 由来の通知設定を検証する |
...
`@pytest.mark.realdevice` は `NYX_REALDEVICE=1` が設定され、必要な capture / serial device が fixture で検出できた場合だけ実行する。`NYX_REALDEVICE` が未設定の場合は skip、環境変数が設定されているのにデバイス検出に失敗した場合は fail とする。通常 CI は `NYX_REALDEVICE` を設定しない。
...
| `pytest.skip` | `@pytest.mark.realdevice` 付きで `NYX_REALDEVICE=1` がない |
...
各テストは shim を触るケースだけ `reset_for_testing()` を fixture で呼び、`global_settings`、`secrets_settings`、`log_manager`、device manager の状態を初期化する。新 Runtime 経路の Runtime、Registry、Port fake、settings/secrets snapshot、logging components はテストごとに生成し、グローバル共有しない。

## 5. テスト方針

本節のテスト名を再設計ドキュメント群の正本とする。実装計画や廃止計画は、本表のテスト名を参照する。

| テスト種別 | テスト名 | 検証内容 |
|------------|----------|----------|
| ユニット | `test_macro_public_import_paths_are_stable` | `MacroBase` / `Command` / `DefaultCommand` / constants / `MacroStopException` の import path を検証する |
| ユニット | `test_command_public_method_names_are_stable` | `Command` の既存メソッド名を検証する |
| ユニット | `test_macro_executor_removed` | `MacroExecutor` の import 互換 shim が残っていないことを検証する |
| 結合 | `test_gui_cli_entrypoints_do_not_import_macro_executor` | GUI/CLI entrypoint の import graph に `MacroExecutor` が含まれない |
| ユニット | `test_macro_public_import_paths_are_stable` | `Button`, `Hat`, `LStick`, `RStick`, `KeyType` を import できる |
| ユニット | `test_registry_loads_convention_package_macro` | package fixture を `MacroDefinition` 化する |
| ユニット | `test_load_failure_is_reported_without_stopping_reload` | 壊れた fixture が reload 全体を止めず診断に残る |
| ユニット | `test_execute_creates_new_instance_each_time` | 実行ごとに `MacroBase` インスタンスが共有されない |
| ユニット | `test_runner_calls_lifecycle_in_order` | `initialize -> run -> finalize` の順序を検証する |
| ユニット | `test_runner_converts_macro_stop_to_cancelled` | 既存 `MacroStopException` を `result.status == RunStatus.CANCELLED` にする |
| ユニット | `test_finalize_error_is_added_without_losing_original_error` | finalize 失敗が元エラー情報を失わせない |
| ユニット | `test_macro_runtime_run_delegates_to_runner_and_closes_ports` | fake Ports で同期実行が成功する |
...
| ユニット | `test_log_sink_dispatcher_snapshot_lock_order` | `sink_lock` 内で sink emit せず、snapshot 後に lock 外配信する |
| ユニット | `test_log_sink_dispatcher_exception_isolation` | `GuiLogSink` 相当の sink が例外を送出しても後続 sink へ配信される |
| ユニット | `test_lock_policy_no_deadlock_under_stress` | registry / runtime / frame / sink の短時間並行操作で deadlock しない |
| ユニット | `test_default_command_press_expands_to_port_sequence` | `press(dur, wait)` が press、cancel-aware wait、release、cancel-aware wait に展開される |
| ユニット | `test_default_command_rejects_legacy_constructor_args` | 旧具象引数が `TypeError` になる |
| ユニット | `test_frame_source_latest_frame_contract` | `latest_frame()` が BGR `uint8` の native size copy を返し、resize は `DefaultCommand.capture()` 側で行う |
| ユニット | `test_frame_source_try_latest_frame_is_nonblocking` | lock 競合時に `try_latest_frame()` が待たずに `None` を返す |
| ユニット | `test_runtime_collects_structured_port_close_warnings` | 複数 Port close 失敗を `CleanupWarning` として `cleanup_warnings` に全件保持し、status を変えない |
| ユニット | `test_fake_frame_source_readiness_failure` | readiness 未達を Runtime が失敗にする |
| ユニット | `test_fake_resource_store_path_escape` | fake resource で root 外参照、drive、UNC を拒否し、Windows 相対 path を許容する |
| ユニット | `test_fake_notification_does_not_expose_secret` | 通知本文、metadata、warning log に webhook、password、token、平文 URL が出ない |
| 結合 | `test_convention_package_and_single_file_macros_load_without_manifest` | 移行後の代表マクロが manifest なしでロードされる |
...
| GUI | `test_main_window_uses_run_handle` | GUI 実行開始が `RunHandle` を保持する |
| GUI | `test_main_window_cancel_calls_handle_cancel` | GUI cancel が `RunHandle.cancel()` を呼ぶ |
| GUI | `test_main_window_poll_updates_status_from_run_result` | GUI 完了監視が `RunResult` を UI 状態へ反映する |
| GUI | `test_gui_log_pane_displays_user_event_from_sink` | `GuiLogSink` の Qt Signal 受信から `LogPane` 行追加までを検証する |
...
| 性能 | `test_cancel_latency_perf` | `safe_point_latency` が 100 ms 以内 |
...
- [x] import/signature 互換テストを最初に追加
- [x] 移行後マクロ fixture を package / single-file / settings ありで用意
- [x] repository macro を manifest あり / なしの新 discovery からロードする結合テストを追加
- [x] Registry / Factory の単体テストを追加
- [x] Runner の lifecycle、例外、中断、finalize テストを追加
- [x] Runtime の同期実行、非同期実行、`RunHandle` テストを追加
- [x] Ports の fake adapter テストを追加
- [x] GUI/CLI integration テストを追加
- [x] hardware tests へ `@pytest.mark.realdevice` を付与
- [x] 性能検証を `tests\perf\` に分離
- [x] スレッド安全性とキャンセル応答の検証を追加
- [x] `uv run pytest tests\unit\` を段階ごとに実行
