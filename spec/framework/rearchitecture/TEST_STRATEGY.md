...
`MacroExecutor` は公開互換契約から明示的に除外する。テストは成功時 `None`、失敗時例外再送出、`macros` / `macro` 属性を保証しない。`test_macro_executor_removed` は `nyxpy.framework.core.macro.executor` の import が `ModuleNotFoundError` になることを期待する。`test_gui_cli_do_not_import_macro_executor` は GUI/CLI entrypoint の import graph に `MacroExecutor` が含まれないことを確認する。
...
| ユニット | `test_gui_cli_do_not_import_macro_executor` | GUI/CLI entrypoint の import graph に `MacroExecutor` が含まれない |
...
| ユニット | `test_runner_normalizes_macro_stop_exception` | 既存 `MacroStopException` を `result.status == RunStatus.CANCELLED` にする |
...
| 結合 | `test_migrated_macro_settings_load_from_explicit_source` | 移行後 settings が manifest または class metadata settings path から読み込まれる |
...
| GUI | `test_gui_cancel_response` | GUI cancel の `cancel_request_latency` が 100 ms 未満である |
