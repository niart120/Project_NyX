...
> **破壊的変更**: `MacroBase` / `Command` / `DefaultCommand` / constants / `MacroStopException` の import と lifecycle は維持する。Resource I/O、settings lookup、legacy auto discovery、`DefaultCommand` 旧コンストラクタ、`MacroExecutor`、GUI/CLI 内部入口、singleton 直接利用、暗黙 fallback は互換維持対象に含めず、移行後テストと削除確認テストで検証する。
...
| `tests\fixtures\macros\manifest_single_file.py` | 新規 | manifest entrypoint を持つ single-file マクロ fixture |
| `tests\fixtures\resources\manifest_package\settings.toml` | 新規 | manifest settings fixture |
...
| 互換対象外 | 旧 settings lookup、legacy auto discovery、`DefaultCommand` 旧コンストラクタ、`MacroExecutor`、GUI/CLI 内部入口、singleton 直接利用、暗黙 dummy fallback | 削除確認テストと新 Runtime 入口テストで固定 |
...
マクロ fixture はフレームワーク公開面だけに依存し、`Command.log()`、`Command.save_img()`、`MacroStopException`、`finalize(cmd)` の維持対象を固定する。settings は manifest settings path、リソースは `resources\<macro_id>\assets` / `runs\<run_id>\outputs` へ移行した状態で検証する。
...
fixture は package 形式、single-file 形式、manifest entrypoint、壊れた macro、settings あり、settings なしを含める。repository macro 互換テストは移行後の代表マクロをロードし、`MacroDefinition` 生成、settings 解決、`initialize -> run -> finalize` の最小 dry-run を検証する。
...
- [ ] repository macro を manifest entrypoint からロードする結合テストを追加
