...
| DefaultCommand | 現行 `Command` の標準実装 import path。再設計後は `ExecutionContext` を受け取る `CommandFacade` の薄い別名または subclass とし、旧コンストラクタ引数は受け付けない。 |
...
| ハードウェア | `test_macro_runtime_realdevice_press_and_capture` | 実機接続時に `CommandFacade` 経由の press と capture が runtime 実行でも動作することを `@pytest.mark.realdevice` で検証する。 |
