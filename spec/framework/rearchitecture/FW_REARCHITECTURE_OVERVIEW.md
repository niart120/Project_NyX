...
| MacroSettingsResolver | manifest settings path と project root 明示設定を解決する専用コンポーネント。画像保存用の `ResourceStorePort` とは分離する。 |
...
- `nyxpy.framework.core.macro.command.DefaultCommand`、constants、`MacroStopException` を互換対象として扱う。旧 settings lookup は互換契約に含めず、manifest settings path と `exec_args` merge の契約を新たに固定する。
...
| 段階 | 内容 | 互換性の扱い |
|------|------|--------------|
| Phase 0 | import/signature 互換テストを追加する。 | 実装変更前に `MacroBase` / `Command` / `DefaultCommand` / constants / `MacroStopException` / manifest settings path と `exec_args` merge の契約を固定する。 |
...
| Phase 5 | Ports / `DefaultCommand(context=...)` を導入し、`DefaultCommand` 内部を Port 委譲へ移行する。 | `Command.type(key: str | KeyCode | SpecialKeyCode)` を含む既存呼び出しを維持する。`Command.stop()` は協調キャンセル優先へ変更するため移行対象として扱う。 |
...
type KeyInput = Button | Hat | LStick | RStick | KeyType | str
...
class Command(ABC):
    @abstractmethod
    def press(self, *keys: KeyInput, dur: float = 0.1, wait: float = 0.1) -> None:
        ...

    @abstractmethod
    def hold(self, *keys: KeyInput) -> None:
        ...

    @abstractmethod
    def release(self, *keys: KeyInput) -> None:
        ...
    ...
    @abstractmethod
    def type(self, key: KeyInput) -> None:
        ...
...
| `macro_settings_root` | `Path` | `project_root` | `MacroSettingsResolver` が manifest settings path と project root 明示設定を解決する起点。 |
