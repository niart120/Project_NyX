...
| MacroExecutor | 削除済みの旧マクロ発見・選択・実行クラス。公開 API・既存マクロ互換契約・移行 adapter に含めず、import shim も提供しない。 |
...
- CLI は `MacroRuntimeBuilder.run()`、GUI は `MacroRuntimeBuilder.start()` と `RunHandle` を使う。

#### 問題

- マクロ発見、実行インスタンス生成、ライフサイクル実行、GUI/CLI の実行制御は `MacroRegistry`、`MacroRuntimeBuilder`、`MacroRuntime`、`MacroRunner` へ分離済みである。
- `MacroDefinition.factory` は実行ごとに新しい `MacroBase` インスタンスを返すため、発見時インスタンス保持には依存しない。
...
| `src\nyxpy\framework\core\macro\executor.py` | 削除 | GUI/CLI/テストの参照を `MacroRuntime` / `MacroRegistry` へ移行した後に削除済み。import 互換 shim は作らない。 |
...
| `src\nyxpy\gui\main_window.py` | 変更 | `MacroRuntimeBuilder.start()` と `RunHandle` を使う GUI adapter へ移行し、`MacroExecutor` 参照を削除する。 |
...
| `tests\unit\framework\macro\test_registry.py` | 新規 | マクロ探索、reload、パッケージ型マクロ、import キャッシュ削除を検証する。 |
...
| `tests\gui\test_main_window.py` | 変更 | GUI adapter が `RunHandle` の完了・中断・失敗を既存 UI 状態へ反映することを検証する。 |
...
`MacroRunner.start()` は core 層では標準ライブラリのスレッド実行を基本とし、Qt へ依存しない。GUI は `RunHandle` を直接 UI スレッドから待たず、Qt adapter で完了通知へ変換する。中断は `CancellationToken` を経由する。既存 `cmd.stop()` の呼び出し可能性は保つが、即時例外送出の意味論は維持しない。
...
再設計後の正経路では `singletons.py` の manager 群を使用しない。`serial_manager`、`capture_manager`、`global_settings`、`secrets_settings` は互換 shim として段階的に廃止し、GUI / CLI の composition root が settings、secrets、device discovery、logging、runtime builder を明示生成して各層へ渡す。`log_manager` / `LogManager` は互換 shim を残さず削除する。`MacroRuntimeBuilder`、`MacroRuntime`、`MacroRegistry`、`RunHandle`、Port 実体は singleton にしない。
...
1. GUI/CLI の composition root が明示 `project_root` で生成した `MacroRegistry.reload()` を呼ぶ。
2. `MacroRegistry` は `project_root / "macros"` または明示された `macros_dir` を走査する。`Path.cwd()` fallback は使わない。
3. 単一ファイル型 `macros\sample.py` とパッケージ型 `macros\sample\__init__.py` を対象にする。
4. reload 時は対象 module とそのサブモジュールを `sys.modules` から削除してから import する。
...
3. 生成失敗は `MacroLoadError` として扱い、GUI/CLI adapter では `RunResult` / ユーザー表示 / 技術ログへ変換する。
...
| パラメータ | 型 | デフォルト | 説明 |
|------------|-----|-----------|------|
| `macros_dir` | `Path` | `project_root / "macros"` | マクロ探索ディレクトリ。`project_root` は GUI/CLI composition root が明示する。 |
...
| `runtime.device_detection_timeout_sec` | `float` | `5.0` | `DeviceDiscoveryService.detect(timeout_sec)` の最大待機秒数。暗黙の非同期 auto register は新 Runtime 経路で使わない。 |
| `runtime.allow_dummy` | `bool` | `False` | テストと明示 dry-run で dummy Port を許可するか。 |
...
| `MacroStopException` | 既存マクロが直接送出した場合、または `check_interrupt` による中断。 | `RunStatus.CANCELLED` へ正規化する。 |
...
再設計後の正経路では `singletons.py` を参照しない。現行 `singletons.py` の `serial_manager`、`capture_manager`、`global_settings`、`secrets_settings` は互換 shim として段階的に廃止し、GUI / CLI の composition root が device discovery、settings / secrets store、logging components、Runtime builder を明示生成する。`log_manager` / `LogManager` は互換 shim を残さず `LoggerPort`、`LogBackend`、`LogSinkDispatcher`、`LogSanitizer` へ置換して削除する。
...
| テスト種別 | テスト名 | 検証内容 |
|------------|----------|----------|
| ユニット | `test_macro_public_import_paths_are_stable` | `MacroBase`、`Command`、`DefaultCommand`、constants、`MacroStopException` が既存 import パスから import できることを検証する。 |
| ユニット | `test_registry_loads_convention_single_file_macro` | 単一ファイル型の `MacroBase` 継承クラスを `MacroRegistry` が発見できることを検証する。 |
| ユニット | `test_registry_loads_convention_package_macro` | パッケージ型の `MacroBase` 継承クラスを `MacroRegistry` が発見できることを検証する。 |
| ユニット | `test_registry_reload_restores_sys_path_even_on_import_error` | reload 時に `sys.path` を恒久変更しないことを検証する。 |
| ユニット | `test_execute_creates_new_instance_each_time` | `definition.factory.create()` が呼び出しごとに別インスタンスを返し、実行間状態を共有しないことを検証する。 |
...
| 結合 | `test_gui_cli_entrypoints_do_not_import_macro_executor` | GUI/CLI/テスト入口が `MacroExecutor` を import せず `MacroRuntime` / `MacroRegistry` を使うことを検証する。 |
| ユニット | `test_macro_executor_removed` | `MacroExecutor` の import 互換 shim を作らず、削除対象として扱うことを検証する。 |
| 結合 | `test_cli_uses_runtime_and_run_result` | CLI 引数から実行引数を作り、runtime 経由でマクロを実行できることを検証する。 |
| GUI | `test_main_window_uses_run_handle` | GUI adapter が実行開始時に `RunHandle` を保持することを検証する。 |
| GUI | `test_main_window_poll_updates_status_from_run_result` | GUI adapter が完了結果を `ControlPane` と status label に反映することを検証する。 |
...
- [x] Phase 1 実装前に import 互換テストを追加
- [x] `MacroRuntime` 系クラスを実装
- [x] GUI/CLI/テストの `MacroExecutor` 参照を削除し、`test_macro_executor_removed` と `test_gui_cli_entrypoints_do_not_import_macro_executor` を追加
- [x] CLI adapter を runtime 経由へ移行
- [x] GUI adapter を runtime 経由へ移行
- [x] Resource I/O と settings lookup の破壊的変更をマクロ移行ガイドへ記載
- [x] 既存マクロを移行ガイドに従って更新し、主要テストが通ることを確認
