...
| LoggerPort | `DefaultLogger` またはテスト用ロガーへログを出力する Port。backend / sink 実装へ直接依存しない |
...
| CLI notification settings | CLI と GUI で通知設定の参照経路が分かれ得る | Discord / Bluesky の有効化と secret 値を secrets snapshot に統一しないと、Runtime 移行後に通知挙動が入口でずれる |
...
ControllerOutputPort / FrameSourcePort / ResourceStorePort / RunArtifactStore / NotificationPort / LoggerPort
    ↓
SerialComm / AsyncCaptureDevice / static resources / NotificationHandler / DefaultLogger
```

`MacroRuntimeBuilder` の所有権は本書に置く。`CONFIGURATION_AND_RESOURCES.md` は `SettingsStore` / `SecretsStore` / `MacroSettingsResolver` の読み込み結果だけを定義し、`RESOURCE_FILE_IO.md` は `MacroResourceScope`、`ResourceStorePort`、`RunArtifactStore`、path guard だけを定義する。GUI/CLI は settings と resource を個別に問い合わせず、実行時は `MacroRuntimeBuilder.build()` を入口にする。
...
| GUI preview の `FrameSourcePort.try_latest_frame()` | UI thread の待機 0 ms。取得できない場合は frame skip し、直近表示を維持 |
| GUI preview 1 tick の処理時間 | 60 FPS 設定で 16 ms 未満、30 FPS 設定で 33 ms 未満。超過率 1% 未満 |
...
| `RunHandle.cancel()` から `result.status == RunStatus.CANCELLED` までの `cancel_result_latency` | マクロが `Command` API 内にいる場合 100 ms 以下 |
...
- GUI preview は UI thread から blocking wait を行わない。`try_latest_frame()` で取得できない場合はその tick の描画を skip し、直近 pixmap を維持する。
...
- `NotificationPort` は通知先ごとの例外を捕捉し、`LoggerPort` に `NYX_NOTIFICATION_FAILED` を持つ警告として記録する。例外は再送出せず、Runtime の成功・失敗判定は通知失敗で変更しない。
- `LoggerPort` は `LogSinkDispatcher` と `LogBackend` の thread-safe 性に委譲する。テスト用実装は list への追記を lock で保護する。
...
`run_start_lock` は実行開始の状態遷移だけを保護し、`MacroRunner.run()` 呼び出し前に必ず解放する。`RunHandle.result()` は完了前なら lock 内で状態だけを確認して `RuntimeError` を送出し、完了後は `RunResult` 参照を取り出してから lock を解放する。`FrameSourcePort.latest_frame()` は lock 内で frame 参照を検証して copy し、resize、crop、grayscale は lock 外の `DefaultCommand.capture()` で行う。`FrameSourcePort.try_latest_frame()` は lock を取得できない場合に `None` を返し、GUI preview の UI thread を待たせない。

GIL あり CPython を前提にしても、`FrameSourcePort` の lock 保持中に OpenCV resize、QImage 生成、disk I/O、ログ出力を行わなければ preview と Runtime の競合は局所化できる。GIL 解除版 Python は初期要件にしない。`test_preview_runtime_frame_source_contention_perf` で preview tick 超過率が 1% 以上、または preview 無効時を基準にした capture p95 latency が 2 倍を超える状態が adapter 改善後も残る場合に限り、別仕様で free-threaded CPython の採用可否、PySide6 / OpenCV wheel 対応、配布手順、実機テストを評価する。
...
`frame_source_for_preview()` と `controller_output_for_manual_input()` は GUI adapter 用の Port 取得 API であり、Qt 型や widget を受け取らない。これらは `MacroRuntimeBuilder` が所有する `DeviceDiscoveryService` と Port factory から、GUI lifetime の preview / manual input 用 Port を返す。実行用 `ExecutionContext` に入る Port と同じ抽象・同じ adapter 契約を使うが、Runtime 実行の `finally` で GUI lifetime の Port を閉じてはならない。underlying device の共有、lock、frame snapshot 配信、二重 open 回避は `FrameSourcePort` / capture adapter と controller adapter の責務とし、`shutdown()` が GUI lifetime の Port と device service を閉じる。
...
`FrameSourcePort.try_latest_frame()` は GUI preview 用の非ブロッキング取得 API である。frame lock を即時取得できない場合、ready 未達の場合、または直近 frame がない場合は `None` を返す。例外を表示 loop へ伝播させず、失敗詳細が必要な場合は adapter 側で rate limit 付き技術ログへ出す。返却する frame は `latest_frame()` と同じく copy とし、GUI 側の resize / QImage 生成は lock 外で行う。
...
`NotificationHandlerAdapter` は現行 `NotificationHandler` を adapter として包む。通知先がない場合は `NoopNotificationAdapter` を使う。抽象は常に `NotificationPort` とし、`NotificationHandler` は adapter の内部実装詳細として扱う。通知先設定の不足や secret 境界違反は Runtime builder 構築時の `ConfigurationError` とし、`NotificationPort.publish()` では扱わない。
...
main()
  ├─ argparse で引数解析
  ├─ logging components / settings / secrets を composition root で明示生成
  ├─ builder = MacroRuntimeBuilder(project_root=..., registry=..., settings_resolver=..., settings=..., secrets=..., device_discovery=..., port_factories=...)
...
CLI から `serial_manager.get_active_device()` と `capture_manager.get_active_device()` を直接呼ばない。検出完了待ち、timeout、dummy 許可は `MacroRuntimeBuilder.run()` の build 手順に集約し、失敗時は `RunResult.failed` と `ErrorInfo` に変換する。Discord / Bluesky 通知設定は secrets snapshot を唯一の入力元とし、CLI 独自の通知設定解釈を持たない。

#### GUI

```text
MainWindow.__init__()
  ├─ logging components / settings / secrets を application lifetime で明示生成
  ├─ builder = MacroRuntimeBuilder(project_root=..., registry=..., settings_resolver=..., settings=..., secrets=...)
  ├─ GUI adapter がデバイス検出を background 起動し、結果 snapshot を builder 入力へ渡す
  ├─ PreviewPane は builder.frame_source_for_preview() の FrameSourcePort を QTimer で参照
...
| `serial_device` | `str` | `""` | settings snapshot / 実行要求 | GUI/CLI で選択する serial device 名 |
| `serial_protocol` | `str` | `"CH552"` | settings snapshot / 実行要求 | `ProtocolFactory` で解決する serial protocol 名 |
| `serial_baud` | `int | None` | `None` | settings snapshot / 実行要求 | 明示 baudrate。`None` は protocol 既定値 |
| `capture_device` | `str` | `""` | settings snapshot / 実行要求 | GUI/CLI で選択する capture device 名 |
...
| `notification.discord.enabled` | `bool` | `False` | secrets snapshot | CLI/GUI/Runtime builder の唯一の通知設定ソース |
| `notification.bluesky.enabled` | `bool` | `False` | secrets snapshot | CLI/GUI/Runtime builder の唯一の通知設定ソース |
...
再設計後の正経路では `singletons.py` の manager 群を参照しない。GUI / CLI の composition root が settings、secrets、device discovery、logging、runtime builder を明示生成し、`MacroRuntimeBuilder` へ依存を渡す。`singletons.py` に既存互換 shim を残す場合でも、新 Runtime 経路から直接参照しない。

| 対象 | 方針 | `reset_for_testing()` での扱い |
|------|------|-------------------------------|
| `serial_manager` / `capture_manager` | 互換 shim。新 Runtime 経路では `DeviceDiscoveryService` と Port factory を使う | shim が残る間だけ再生成し、検出済み cache を破棄する |
| `global_settings` / `secrets_settings` | 互換 shim。新 Runtime 経路では composition root が `SettingsStore` / `SecretsStore` を読み snapshot を渡す | shim が残る間だけ snapshot と lock 状態を初期化する |
| `log_manager` / `LogManager` | 廃止。`LoggerPort`、`LogBackend`、`LogSinkDispatcher`、`LogSanitizer` へ分解し、互換 shim は残さない | 対象外。新 Runtime 経路とテスト fixture は logging components を明示生成・破棄する |
| device discovery cache | `DeviceDiscoveryService` が所有する明示依存 | 完了・失敗・timeout 状態を破棄する |
| `MacroRuntimeBuilder` / `MacroRuntime` / `MacroRegistry` / `RunHandle` / Port 実体 | singleton にしない | テスト fixture または GUI / CLI composition root が lifetime を管理する |
...
| CLI notification settings | Runtime builder が secrets snapshot から `NotificationPort` を構築する。CLI 独自設定や settings snapshot 由来の secret 値は持たない |
...
| 性能 | `test_preview_runtime_frame_source_contention_perf` | Runtime capture と GUI preview 同時利用時に preview tick 超過率 1% 未満、capture p95 latency が preview 無効時の 2 倍未満 |
