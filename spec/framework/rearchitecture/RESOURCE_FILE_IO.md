...
> **破壊的変更**: 既存ユーザーマクロの公開互換契約に対してはなし。既存 `cmd.load_img()` / `cmd.save_img()` と `static\<macro_name>\...` 参照は互換層で維持する。ただし成果物の標準保存先、Resource Store 内部実装、singleton 直接利用、暗黙 fallback は互換維持対象に含めず、新 API へ置換または削除する。
...
`MacroResourceScope.assets_roots` が複数 root を持つ理由は、標準 assets、macro package 同梱 assets、legacy static assets を同時に検索するためである。例えば `frlg_id_rng` は次の順で検索する。
...
この順序により、新配置へ移した template を優先しつつ、移行前の `static\<macro_name>` を既存マクロ互換として残せる。複数 root は読み込み専用候補であり、保存先の分散には使わない。
