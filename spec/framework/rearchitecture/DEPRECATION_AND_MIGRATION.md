...
| settings legacy fallback | 既存 settings を manifest または class metadata settings path へ移行する | `MacroSettingsResolver`, `macro.toml [macro].settings`, `MacroBase.settings_path` | マクロ側の settings 配置修正が必要。manifest は高度機能が必要な場合だけ追加 | `test_macro_settings_resolver_does_not_read_legacy_static_settings`, `test_migrated_macro_settings_load_from_explicit_source` | 移行ガイド作成 → 代表マクロ更新 → static/cwd fallback 削除 |
...
|------|----------|----------|----------|--------------|--------|
| `singletons.py` のグローバル manager 直接利用 | GUI/CLI/Command/Runtime が `serial_manager` / `capture_manager` / `log_manager` / `global_settings` / `secrets_settings` を直接参照しない | `MacroRuntimeBuilder`, `ControllerOutputPort`, `FrameSourcePort`, `DeviceDiscoveryService`, `SettingsStore`, `SecretsStore`, `LoggerPort` | 既存マクロへの影響なし。GUI/CLI の設定適用経路は破壊的変更を許容する | `test_cli_uses_macro_runtime_builder`, `test_main_window_starts_runtime_and_updates_status`, `test_new_runtime_does_not_import_singletons`, `reset_for_testing` 関連テスト | 直接参照箇所を洗い出し → composition root 経由へ置換 → 旧 GUI/CLI 参照を削除 → 新規直接利用を禁止 |
...
| loguru 直結の `LogManager` グローバル初期化 | `LogManager` クラスと `log_manager` グローバルを削除し、Runtime は composition root から `LoggerPort` を受け取る | `LoggerPort`, `DefaultLogger`, `LogSinkDispatcher`, `LogBackend`, `LogSanitizer` | `Command.log()` は維持。保存ログ形式と GUI 表示イベントは変わる | `test_default_logger_emits_structured_log_with_run_context`, `test_gui_sink_exception_is_logged_and_ignored`, `test_technical_log_masks_secrets`, `test_legacy_log_manager_removed` | 構造化ログ API 追加 → dispatcher/backend/sanitizer 分離 → GUI sink 分離 → `LogManager` と module global を削除 |
