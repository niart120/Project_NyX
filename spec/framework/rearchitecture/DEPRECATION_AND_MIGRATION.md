...
既存マクロ互換を維持するため、廃止対象を公開互換契約から明確に外し、代替経路とテストゲートを先に固定する必要がある。特に `MacroExecutor` は再設計で削除する旧実装として扱い、GUI/CLI/テストの参照を解消した後に import shim を作らず削除する。
...
| Legacy adapter | 既存マクロが直接 import する API のみ。`MacroExecutor` は含めない | lifecycle 二重実装、発見時インスタンス保持、`Path.cwd()` 固定探索 |
