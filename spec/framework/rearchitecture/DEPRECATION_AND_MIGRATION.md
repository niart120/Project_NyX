...
| Resource I/O legacy static 互換 | 移行ガイドに従い既存 assets / artifacts を `resources\<macro_id>\assets` と `resources\<macro_id>\artifacts` へ移行する | `ResourceStorePort`, `RunArtifactStore`, `ResourceRef` | マクロ側の path 指定修正が必要 | `test_runtime_saves_command_images_to_resource_artifacts`, `test_local_run_artifact_store_saves_run_artifacts_without_stripping_macro_prefix` | 移行ガイド作成 → 代表マクロ更新 → legacy static read/write 経路削除 |
