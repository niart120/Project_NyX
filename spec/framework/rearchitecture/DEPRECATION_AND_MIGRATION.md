...
| `DefaultCommand` 旧コンストラクタ | GUI/CLI/テストが `MacroRuntimeBuilder` から得た context を渡す | `DefaultCommand(context=...)` | 既存マクロへの影響なし。`DefaultCommand` 直接生成コードは修正が必要 | `test_default_command_rejects_legacy_constructor_args`, `test_cli_uses_runtime_and_run_result` | Builder 経由へ移行 → 旧コンストラクタ引数削除 |
...
|------|----------|----------|----------|--------------|--------|
| `singletons.py` のグローバル manager 直接利用 | GUI/CLI/Command/Runtime が `serial_manager` / `capture_manager` / `log_manager` / `global_settings` / `secrets_settings` を直接参照しない | `MacroRuntimeBuilder`, `ControllerOutputPort`, `FrameSourcePort`, `DeviceDiscoveryService`, `SettingsStore`, `SecretsStore`, `LoggerPort` | 既存マクロへの影響なし。GUI/CLI の設定適用経路は破壊的変更を許容する | `test_cli_uses_runtime_and_run_result`, `test_gui_start_uses_runtime_handle`, `test_new_runtime_does_not_import_singletons`, `reset_for_testing` 関連テスト | 直接参照箇所を洗い出し → composition root 経由へ置換 → 旧 GUI/CLI 参照を削除 → 新規直接利用を禁止 |
