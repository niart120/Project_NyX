...
- `MacroExecutor` は既存マクロ互換 API ではない。旧 GUI/CLI/テスト入口として残す場合だけ一時 adapter とし、互換ゲートを満たせるなら廃止候補化を許可する。
...
| 10 | MacroExecutor 廃止判断または adapter 縮退 | フェーズ 8、9 |
...
| Signature gate | `initialize(cmd, args)`、`run(cmd)`、`finalize(cmd)`、主要 `Command` メソッドの呼び出し互換を維持する。`MacroExecutor.execute()` は残す場合だけ legacy executor gate で検証する |
...
| Legacy executor gate | `MacroExecutor` を残す場合だけ、`reload_macros()`、`set_active_macro()`、`execute()` の成功時 `None` と失敗時例外再送出を一時 adapter として維持する |
...
- Legacy executor gate（`MacroExecutor` を残す場合）の成功時 `None` と失敗時例外再送出
...
### 4.10 フェーズ 10: MacroExecutor 廃止判断または adapter 縮退
...
| テスト | `test_macro_executor_delegates_to_runtime`, `test_macro_executor_execute_keeps_legacy_none_return_if_kept`, `test_macro_executor_has_no_lifecycle_duplication`, `test_legacy_imports_keep_macro_base_command_contract` |
...
本計画では `MacroExecutor` の即削除を完了条件にしない。ただし既存マクロ互換 API には含めず、旧 GUI/CLI/テスト入口として必要な期間だけ一時 adapter として残す。互換ゲートが成立し、外部利用の非推奨期間を終えた場合は削除できる。
廃止候補の最終判断は `DEPRECATION_AND_MIGRATION.md` に従い、`MacroExecutor` だけでなく singleton 直接利用、暗黙 device detection、dummy fallback、`Path.cwd()` fallback、GUI/CLI 個別 Command 構築も同じゲートで扱う。
...
| 5 | `uv run pytest tests\integration\test_macro_runtime_legacy_executor.py tests\integration\test_existing_macros_compat.py` | 既存マクロと旧 executor の互換確認 |
...
| 10 | `MacroExecutor` adapter thinness、legacy return/exception compatibility |
