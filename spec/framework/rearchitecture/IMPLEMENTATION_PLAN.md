...
| DefaultCommand | 既存 import path を維持する `Command` 実装。生成は `DefaultCommand(context=...)` に統一し、旧具象引数コンストラクタは残さない |
...
| DefaultCommand | 既存 `Command` API を維持し、`ExecutionContext` 経由で Ports へ委譲する実装。追加の別 `Command` 実装クラスは置かない |
| MacroSettingsResolver | manifest settings を解決するコンポーネント。Resource File I/O から分離し、旧 static/cwd fallback を持たない |
...
| マクロ配置・リソース移行 | `static` と legacy discovery に依存 | `macro.toml`、manifest settings、assets root、run outputs へ移行 |
| 互換契約の検証 | 一部の executor テストと暗黙の import 維持に依存 | import、シグネチャ、Command API、manifest settings、代表マクロロードをゲート化 |
...
| `tests\unit\framework\macro\test_registry.py` | 新規 | Registry、manifest entrypoint、診断、設定解決を検証 |
...
| `tests\integration\test_migrated_macros_compat.py` | 新規 | 移行後代表マクロのロード、設定マージ、lifecycle 互換を検証 |
...
| `src\nyxpy\framework\core\macro\entrypoint_loader.py` | 新規 | manifest entrypoint で package / single-file マクロを解決 |
...
実装前提とフェーズ 1 の成果物を分ける。実装前提は仕様確定、ベースライン確認、実機不要テストと実機テストの分離、維持対象 import / lifecycle に起因するマクロ本体変更を発生させない制約である。フェーズ 1 の成果物は、import / signature / manifest settings / 代表マクロロード / `MacroExecutor` 削除確認のテスト追加であり、フレームワーク本体の挙動は変更しない。
...
| 6A | Resource File I/O 再設計 | フェーズ 5、6 |
| 7 | Logging Framework 再設計 | フェーズ 3、5、6 |
| 8 | Ports/Adapters 導入 | フェーズ 4、5、6、6A、7 |
| 9 | CLI 移行 | フェーズ 5、7、8 |
...
| Settings gate | manifest settings path を読み、`exec_args` が file settings より優先される。旧 static/cwd fallback は読まない |
| Builder ownership gate | GUI/CLI は settings と resource を個別解決せず、`MacroRuntimeBuilder.build()` を通して `ExecutionContext` を得る |
| Migrated macro gate | 移行後の代表マクロ `frlg_id_rng`、`frlg_initial_seed`、`frlg_gorgeous_resort`、`frlg_wild_rng` が manifest entrypoint からロードされる |
...
| 対象ファイル | `tests\unit\framework\macro\test_import_contract.py`, `tests\integration\test_migrated_macros_compat.py`, 既存 executor / GUI reload テスト |
| 完了条件 | import gate、signature gate、manifest settings gate、migrated macro gate が自動テストで判定できる |
| テスト | `uv run pytest tests\unit\framework\macro\test_import_contract.py`, `uv run pytest tests\integration\test_migrated_macros_compat.py`, `uv run pytest tests\integration\test_macro_runtime_entrypoints.py` |
...
- manifest settings path と `exec_args` 優先マージ
- 移行後代表マクロの manifest entrypoint ロード
...
| 完了条件 | manifest entrypoint で package / single-file マクロを `MacroDefinition` として登録でき、`definition.factory.create()` が毎回新しい `MacroBase` インスタンスを返す |
...
| 項目 | 内容 |
|------|------|
| 目的 | manifest settings 解決と画像リソース保存・読み込みを分離し、path escape と書き込み失敗を検出する |
...
| 完了条件 | manifest settings と `exec_args` 優先マージが維持され、`ResourceStorePort` は settings TOML を探索しない |
...
| ロールバック方針 | `load_macro_settings()` を manifest settings resolver 経由に限定し、旧 static/cwd fallback は戻さない。`ResourceStorePort` の導入範囲を画像入出力だけに限定する |

本フェーズ完了後、旧 settings 配置は読まない。移行が必要なマクロは `macro.toml` の `[macro].settings` へ settings path を明示する。

### 4.6A フェーズ 6A: Resource File I/O 再設計
...
その他の廃止候補は `DEPRECATION_AND_MIGRATION.md` に従い、singleton 直接利用、暗黙 device detection、dummy fallback、`Path.cwd()` fallback、GUI/CLI 個別 Command 構築、legacy auto discovery、旧 settings lookup を個別に扱う。

### 4.12 フェーズ 12: ドキュメント・移行ガイド整理

| 項目 | 内容 |
|------|------|
| 目的 | 実装後の新 API、維持する互換契約、manifest entrypoint、マクロ移行手順、CLI/GUI 移行後の運用、Mermaid 図を文書化する |
...
移行ガイドでは、Resource I/O、settings、manifest entrypoint、single-file マクロの明示 entrypoint、`DefaultCommand` 直接生成コードの修正を明記する。Overview から `ARCHITECTURE_DIAGRAMS.md` と `MACRO_MIGRATION_GUIDE.md` へ到達できる状態を維持し、Logging / Resource File I/O / Deprecation の別建て仕様への参照を更新する。
...
| 5 | `uv run pytest tests\integration\test_macro_runtime_entrypoints.py tests\integration\test_migrated_macros_compat.py` | 移行後マクロと新 Runtime 入口の互換確認 |
...
| フェーズ | 必須ゲート |
|----------|------------|
| 1 | Import gate、signature gate、manifest settings gate、migrated macro gate |
...
| 4 | Port Protocol / ABC、fake adapter、不要な中間 facade がないこと |
| 5 | `ExecutionContext` contract、Runtime sync/async、`RunHandle` contract、GUI/CLI の Runtime 入口 |
| 6 | settings resolver、resource path validation、write/read error normalization、secret masking |
| 6A | resource scope、run artifact store、static 互換削除、atomic write、path guard |
| 7 | logger port、sink dispatch、run context、user/technical split、legacy log API 削除 |
| 8 | adapter integration、frame readiness、dummy policy、hardware boundary |
| 9 | CLI args compatibility、device detection wait、notification settings source、exit code |
| 10 | GUI start/cancel/finish、UserEvent display、thread boundary、window close handling |
| 11 | `MacroExecutor` 削除確認、GUI/CLI の新 Runtime 入口確認 |
| 12 | 文書と実装の差分確認、コード例がある場合の該当テスト |

### 5.3 マクロ移行後の合格条件

- 代表マクロが manifest entrypoint から import、registry 登録、settings 解決、`initialize -> run -> finalize` 実行へ進める。
- 既存 `Command` メソッド呼び出しが `TypeError` にならない。
- 既存 `MacroStopException` を直接送出するマクロが中断として扱われる。
- manifest settings path の settings が読み込まれ、CLI/GUI 実行引数で上書きできる。
...
- [x] 12 フェーズと 6A 補助フェーズを実装順序として定義
...
- [ ] フェーズ 6A の Resource File I/O 再設計を実装
- [ ] フェーズ 7 の Logging Framework 再設計を実装
