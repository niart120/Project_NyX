...
> **関連ドキュメント**: `FW_REARCHITECTURE_OVERVIEW.md`, `MACRO_COMPATIBILITY_AND_REGISTRY.md`, `RUNTIME_AND_IO_PORTS.md`, `ERROR_CANCELLATION_LOGGING.md`
...
| MacroExecutor | 既存 GUI/CLI とテストの互換入口。最終的には `MacroRuntime` へ委譲する adapter として縮退させる |
| MacroRegistry | マクロの発見、安定 ID、別名、診断、設定解決を管理し、実行インスタンスを保持しないコンポーネント |
| MacroFactory | `MacroDescriptor` または `MacroDefinition` から実行ごとに新しい `MacroBase` インスタンスを生成するコンポーネント |
...
| MacroSettingsResolver | manifest settings と `static\<macro_name>\settings.toml` 互換を解決するコンポーネント。画像リソース I/O から分離する |
...
- `FW_REARCHITECTURE_OVERVIEW.md`、`MACRO_COMPATIBILITY_AND_REGISTRY.md`、`RUNTIME_AND_IO_PORTS.md`、`ERROR_CANCELLATION_LOGGING.md` の方針を実装の正とする。
...
- `MacroExecutor` はユーザーマクロが直接依存する公開面を維持できる範囲で、抜本改修、置換、廃止候補化を許可する。
...
| `tests\integration\test_macro_runtime_legacy_executor.py` | 新規 | `MacroExecutor` 経由の既存実行が Runtime 委譲後も互換であることを検証 |
...
| `src\nyxpy\framework\core\macro\executor.py` | 変更 | `MacroExecutor` を Runtime 委譲 adapter へ縮退 |
| `src\nyxpy\framework\core\macro\registry.py` | 新規 | `MacroRegistry`、`MacroDescriptor`、`MacroFactory`、`MacroManifest`、診断モデルを定義 |
...
| `src\nyxpy\framework\core\logger\log_manager.py` | 変更 | 構造化ログと GUI 表示イベントを追加 |
...
本書は仕様の追加定義ではなく、既存仕様を壊さずに実装する順序と検証ゲートを定義する。各フェーズは「互換テストを追加し、最小の実装単位を導入し、既存入口を adapter に寄せ、最後に旧実装を縮退判断する」流れで進める。
...
| 6 | Ports/Adapters 導入 | フェーズ 4、5 |
| 7 | CLI 移行 | フェーズ 4、6 |
| 8 | GUI 移行 | フェーズ 4、6、7 の知見 |
| 9 | MacroExecutor 廃止判断または adapter 縮退 | フェーズ 7、8 |
| 10 | ドキュメント・移行ガイド整理 | 全フェーズ |
...
| ゲート | 判定内容 |
|--------|----------|
| Import gate | `MacroBase`、`Command`、`DefaultCommand`、`MacroExecutor`、`MacroStopException`、constants が既存 path から import できる |
| Signature gate | `initialize(cmd, args)`、`run(cmd)`、`finalize(cmd)`、`MacroExecutor.execute(cmd, exec_args={})`、主要 `Command` メソッドの呼び出し互換を維持する |
...
| Legacy executor gate | `MacroExecutor.reload_macros()`、`set_active_macro()`、`execute()` の成功時 `None` と失敗時例外再送出を維持する |
...
| `refactor(macro): settings 解決と resource I/O を分離` | settings TOML 探索と画像リソース保存の責務混在を解消するため |
...
| `refactor(macro): MacroExecutor を互換 adapter に縮退` | 旧入口の互換を維持しつつ、ライフサイクル二重実装をなくすため |
| `docs(framework): 再設計の移行ガイドを整理` | マクロ作者と保守者が新旧方式の境界を確認できるようにするため |

### 3.5 ロールバック原則

- 互換ゲートが落ちた場合は、そのフェーズの実装コミットを戻し、テスト追加コミットは残す。
- `MacroExecutor`、`DefaultCommand`、CLI、GUI は旧経路へ戻せるよう、移行直後は adapter 分岐を小さく保つ。
...
- `MacroExecutor.execute()` の成功時 `None` と失敗時例外再送出
...
| 完了条件 | legacy package、legacy single file、manifest opt-in を descriptor として登録でき、`MacroFactory.create()` が毎回新しい `MacroBase` インスタンスを返す |
...
この段階では `MacroExecutor` の lifecycle 実行を完全には置き換えなくてよい。ただし `reload_macros()` の探索結果と `macros` 互換 facade は Registry から作れる状態にする。
...
| 対象ファイル | `src\nyxpy\framework\core\runtime\runtime.py`, `context.py`, `handle.py`, `builder.py`, `src\nyxpy\framework\core\macro\command.py`, `executor.py`, `tests\unit\framework\runtime\test_execution_context.py`, `test_command_facade.py`, `tests\integration\test_macro_runtime_legacy_executor.py` |
| 完了条件 | `MacroRuntime.run(context)` と `start(context)` が `MacroRunner` に委譲し、`MacroExecutor.execute()` は Runtime 委譲でも旧戻り値・例外互換を保つ |
| テスト | `test_execution_context_shallow_copies_args_and_metadata`, `test_run_handle_wait_done_result_contract`, `test_run_handle_cancel_requests_token`, `test_macro_executor_delegates_to_runtime_without_breaking_api`, `test_command_facade_press_delegates_to_controller_port`, `test_command_facade_capture_resizes_crops_and_grayscales` |
| リスク | `ExecutionContext` が `Command` を保持する、Runtime と Runner が lifecycle を二重実装する、Port close 失敗が本体失敗を上書きする |
| ロールバック方針 | `MacroExecutor` の Runtime 委譲だけを戻し、Runtime クラスは新 API として残す。`DefaultCommand` は旧コンストラクタ経路を優先する |
...
### 4.9 フェーズ 9: MacroExecutor 廃止判断または adapter 縮退

| 項目 | 内容 |
|------|------|
| 目的 | CLI/GUI 移行後に `MacroExecutor` の残存責務を確認し、廃止候補にするか、互換 adapter として縮退維持するかを決める |
| 対象ファイル | `src\nyxpy\framework\core\macro\executor.py`, `src\nyxpy\framework\core\macro\__init__.py`, `tests\integration\test_macro_runtime_legacy_executor.py`, 関連ドキュメント |
| 完了条件 | `MacroExecutor` が lifecycle、`RunResult` 生成、Ports 準備を二重実装していない。残す場合は Runtime への薄い adapter であることがテストされる |
| テスト | `test_macro_executor_delegates_to_runtime`, `test_macro_executor_execute_keeps_legacy_none_return`, `test_macro_executor_lifecycle_compat`, `test_legacy_imports_keep_macro_base_command_executor` |
| リスク | 既存外部コードが `MacroExecutor.macros` や `macro` 属性に依存している可能性がある |
| ロールバック方針 | 廃止を急がず、`DeprecationWarning` 付き adapter として維持する。削除判断は別の仕様と計画で扱う |

本計画では `MacroExecutor` の即削除を完了条件にしない。既存マクロの依存に関わらない範囲で内部を置換し、公開面は互換 adapter として残す判断を優先する。

### 4.10 フェーズ 10: ドキュメント・移行ガイド整理

| 項目 | 内容 |
|------|------|
| 目的 | 実装後の新旧 API、互換契約、manifest opt-in、CLI/GUI 移行後の運用を文書化する |
| 対象ファイル | `README.md`, `docs\`, `spec\framework\rearchitecture\*.md`, 必要に応じてマクロ作者向け移行ガイド |
| 完了条件 | 既存マクロは変更不要であること、新 API を使う場合の入口、旧 API の非推奨範囲、実機テストの実行条件が読める |
...
移行ガイドでは、既存マクロは変更不要、新 manifest は opt-in、旧 settings fallback は維持、Runtime API は新規フレームワーク利用者向けであることを明記する。
...
| 4 | `uv run pytest tests\integration\test_macro_runtime_legacy_executor.py tests\integration\test_existing_macros_compat.py` | 既存マクロと旧 executor の互換確認 |
| 5 | `uv run pytest tests\integration\test_cli_runtime_adapter.py` | CLI 移行確認 |
| 6 | `uv run pytest tests\gui\test_main_window_runtime_adapter.py` | GUI 移行確認 |
| 7 | `uv run pytest tests\perf\test_macro_discovery_perf.py` | discovery 性能の確認 |
| 8 | `uv run pytest tests\hardware\ -m realdevice` | 実機接続時の確認。通常ゲートからは分離 |
| 9 | `uv run ruff check .` | 静的検査 |
| 10 | `uv run ruff format . --check` | フォーマット確認 |
...
| 4 | `ExecutionContext` contract、Runtime sync/async、`RunHandle` contract、Legacy executor delegation |
| 5 | settings resolver、resource path validation、write/read error normalization、secret masking |
| 6 | fake Port unit tests、adapter integration、frame readiness、dummy policy、structured logging |
| 7 | CLI args compatibility、device detection wait、notification settings source、exit code |
| 8 | GUI start/cancel/finish、GUI log event、thread boundary、window close handling |
| 9 | `MacroExecutor` adapter thinness、legacy return/exception compatibility |
| 10 | 文書と実装の差分確認、コード例がある場合の該当テスト |
...
- `MacroExecutor.execute()` を使う既存コードは成功時 `None`、失敗時例外再送出のまま動作する。
...
- [x] `MacroExecutor` を抜本改修・置換・廃止候補にできる範囲を明記
...
- [ ] フェーズ 6 の Ports/Adapters を実装
- [ ] フェーズ 7 の CLI adapter を実装
- [ ] フェーズ 8 の GUI adapter を実装
- [ ] フェーズ 9 の `MacroExecutor` 縮退判断を実施
- [ ] フェーズ 10 のドキュメント・移行ガイドを整理
