...
| `src\nyxpy\framework\core\macro\command.py` | 変更 | `Command` / `DefaultCommand` import 互換を維持し、内部を `CommandFacade` / Ports へ段階移行。`DefaultCommand` 旧コンストラクタは削除 |
...
| 完了条件 | import gate、signature gate、settings gate、migrated macro gate が自動テストで判定できる |
...
| 完了条件 | manifest entrypoint で package / single-file マクロを `MacroDefinition` として登録でき、`MacroFactory.create()` が毎回新しい `MacroBase` インスタンスを返す |
...
| フェーズ | 必須ゲート |
|----------|------------|
| 1 | Import gate、signature gate、settings gate、migrated macro gate |
...
| 5A | resource scope、run artifact store、static 互換削除、atomic write、path guard |
