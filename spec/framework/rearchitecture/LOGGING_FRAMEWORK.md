...
旧 `LogManager.add_handler()` で登録された callable は `LegacyStringSink` に包む。`LegacyStringSink` は `TechnicalLog` を既存の 1 行文字列へ整形して handler へ渡し、`UserEvent` は扱わない。handler が例外を送出した場合、`LogManager` は `sink.emit_failed` の `TechnicalLog` を残し、例外を呼び出し元や後続 sink へ伝播させない。

| 入力 | LegacyStringSink の処理 | 例外時 |
|------|------------------------|--------|
| `TechnicalLog` | 既存ログ形式の 1 行文字列へ整形し handler に渡す | `sink.emit_failed` を記録し、後続 sink へ配信継続 |
| `UserEvent` | 処理しない | なし |
| `flush()` / `close()` | handler が対応していれば呼ぶ | `sink.emit_failed` を記録し、Runtime result は変更しない |
...
| ユニット | `test_legacy_string_sink_exception_isolated` | 旧 handler 例外が `sink.emit_failed` に変換され、後続 sink が呼ばれる |
