...
> **破壊的変更**: 旧 `log_manager.log(level, message, component="")`、旧 handler API、GUI/CLI 内部のログ接続、singleton 直接利用は互換維持対象に含めず、`LoggerPort` / `LogSink` / `UserEvent` / `TechnicalLog` へ置換または削除する。
...
| 用語 | 定義 |
|------|------|
| LogManager | `LoggerPort` と sink 管理を実装するロギング基盤。旧 `log_manager.log()` 互換 adapter は持たない |
...
| `src\nyxpy\framework\core\logger\log_manager.py` | 変更 | sink 管理、context bind、backend 初期化、secret mask を実装。旧 `log_manager.log()` 互換 adapter は実装しない |
...
旧 `log_manager` グローバル、`LogManager.log()`、`set_level()`、`set_console_level()`、`set_file_level()`、`add_handler()`、`set_custom_handler_level()`、`remove_handler()` は互換維持対象に含めない。呼び出し元を新 API へ置換した後に削除し、`LegacyStringSink` や互換 adapter は作らない。
...
`LogManager.__init__()` は状態初期化だけを行い、backend handler、ファイル、標準出力を作成しない。アプリ起動時に CLI/GUI/テスト fixture が `configure()` を呼び、sink を登録する。未設定状態で `LoggerPort.user()` / `technical()` が呼ばれた場合は `LoggingConfigurationError` とし、成功扱いの fallback は作らない。
...
`LogManager` の sink 登録、解除、level 更新、配信先 snapshot 作成は `sink_lock` で保護する。lock 内では sink の一覧と level 判定に必要な immutable snapshot だけを作り、`emit_*()`、`flush()`、`close()`、Qt Signal emit、ファイル I/O は lock 外で実行する。

| lock 名 | 種別 | 保護対象 | 取得順 | timeout | timeout 時の扱い | 保持してはいけない処理 | テスト名 |
|---------|------|----------|--------|---------|------------------|------------------------|----------|
| `sink_lock` | `threading.RLock` | sink registry、handler registry、level、配信先 snapshot | 全体 5 番目。`frame_lock` より後、他 lock 取得中の logging 呼び出しでは取得しない | 1 秒 | `LogSinkError(code="logging.sink_lock_timeout")` を fallback stderr へ出し、呼び出し元へ再送出しない | sink `emit_*()`、backend write、Qt Signal emit、retention cleanup | `test_log_manager_sink_snapshot_lock_order` |
...
旧 `LogManager.add_handler()` / `remove_handler()` / `set_custom_handler_level()` は実装しない。GUI は `GuiLogSink`、テストは `TestLogSink`、CLI は console sink を使う。旧 callable handler を包む `LegacyStringSink` は作らず、呼び出し元を `LogSink` へ移行する。
...
`log_manager` グローバルインスタンスは互換のため維持しない。アプリ起動時に必要な lifetime で `LogManager` を生成し、`MacroRuntimeBuilder` へ `LoggerPort` として渡す。`singletons.py` の `reset_for_testing()` が存在する場合も、LogManager のグローバル状態ではなく、テストごとの `TestLogSink` と `LogManager` インスタンスを初期化する。
