...
| RotationPolicy | ログファイルのサイズ上限、バックアップ数、保持日数をまとめた値 |
...
旧ローテーションは現行ファイルと `.1` だけを扱う方式で、保持できる履歴数と最大容量が設定として明確ではなかった。実行単位ログにもサイズ上限がなく、長時間のマクロ実行で 1 ファイルが肥大化する余地があった。
...
| コマンド詳細 DEBUG ログ | 既定で出力 | 既定で抑制し、`logging.command_debug_enabled` で明示有効化 |
| ユーザーイベントの技術ログ混入 | `UserEvent` を `TechnicalLog` へ自動複製 | 自動複製しない |
| ログファイル容量 | 現行 + `.1`、実行ログはサイズ上限なし | 現行 + `backup_count` 世代、実行ログも `file_max_bytes` でローテーション |
...
| `src\nyxpy\framework\core\logger\rotation.py` | 新規 | `RotationPolicy` と有界ローテーション、保持期間 cleanup を定義 |
...
`UserEvent` を `TechnicalLog` へ自動複製しない点は出力内容の破壊的変更である。`framework.jsonl` をユーザーイベントの監査ログとして読んでいた呼び出し元は、`nyxpy.log` または `runs\{yyyyMMdd}\{run_id}.jsonl` を参照する。旧挙動を復元する互換 shim は追加せず、技術ログとして必要なイベントは呼び出し元が `logger.technical()` で明示的に出力する。
...
`LoggerPort.user()` は `UserEvent` だけを生成し、`TechnicalLog` へ自動複製しない。ユーザーに見せた文言は `TextFileLogSink` と `RunJsonlFileSink` の `emit_user()` 経路で保存する。`LoggerPort.technical()` は障害調査用の `TechnicalLog` だけを生成し、GUI/CLI のマクロログへは出さない。
...
| sink | GUI 層の `GuiLogSink`、CLI presenter、人間向け file、実行単位 JSONL | framework JSONL backend、人間向け file、console debug |

#### FrameworkError のログ詳細

`LoggerPort.technical(..., exc=...)` に `FrameworkError` が渡された場合、`DefaultLogger` は例外の構造化情報を `TechnicalLog.event.extra` に追加する。呼び出し元が同名 key を明示した場合は呼び出し元の値を優先し、logger 側で上書きしない。

| extra key | 値 | 説明 |
|-----------|----|------|
| `error_kind` | `FrameworkError.kind.value` | GUI/CLI の分類と集計に使うエラー種別 |
| `error_code` | `FrameworkError.code` | `ERROR_CANCELLATION_LOGGING.md` の error code catalog に定義された安定コード |
| `error_component` | `FrameworkError.component` | 例外の発生元コンポーネント。ログ発行元 `component` とは別に保持する |
| `recoverable` | `FrameworkError.recoverable` | リトライ可能性の目安 |
| `error_details` | `FrameworkError.details` | JSON 化可能な詳細。パス、設定 key、macro_id は許可するが secret 値は入れない |

設定読み込み失敗のように Runtime 開始前で `run_id` が未確定の場合も、`TechnicalLog` は `error_code` と `error_details` を保持する。例: `MacroSettingsResolver` が `resource:settings.toml` を解決したがファイルが存在しない場合、例外 message は `macro_id`、元の `settings_path`、解決後の `resolved_path` を含め、`error_code` は `NYX_SETTINGS_NOT_FOUND` とする。GUI/CLI の表示文言は短く保ち、詳細は `logs\framework.jsonl` の `runtime.start_failed` または `cli.unhandled` の `extra` と traceback で確認する。
...
| `command.log` | `Command.log()` | 明示的に `technical()` へ出した場合のみ | マクロ作者が指定した文言 |
...
| `runtime.start_failed` | GUI / CLI composition root | `FrameworkError` の `error_code`、`error_details`、例外型、traceback。`run_id` 未確定の場合は `macro_id` も `extra.error_details` で補う | `マクロを開始できません` |
...
|------|-----------|------|----------|-----------|------|
| 人間向けログ | `logs\nyxpy.log` | text | 10 MB / 3 世代 | 14 日 | 開発中の tail と簡易確認。UserEvent と TechnicalLog を保存 |
| 実行単位構造化ログ | `logs\runs\{yyyyMMdd}\{run_id}.jsonl` | JSON Lines | 10 MB / 3 世代 | 30 日 | マクロ実行の調査、GUI/CLI の相関。run_id 付き UserEvent と TechnicalLog を保存 |
| framework 構造化ログ | `logs\framework.jsonl` | JSON Lines | 10 MB / 3 世代 | 14 日 | 起動、設定、デバイス検出、通知失敗などの TechnicalLog を保存 |

保持期間を過ぎたファイルは composition root が backend / sink を明示生成する時点、またはローテーション判定時に削除対象としてよい。削除失敗は `log.retention_cleanup_failed` の技術ログに残し、アプリ起動は継続する。ファイル書き込みに失敗した場合、該当 sink を無効化して console sink へ警告を出す。

ローテーションは `RotationPolicy(max_bytes, backup_count, retention_days)` を使い、現行ファイルを `.1`、既存 `.1` を `.2` のように番号付きで繰り上げる。`backup_count` を超える世代は削除する。複数プロセスが同じログファイルへ同時書き込みする場合の排他は未対応であり、別途設計判断する。
...
| `logging.file_level` | `str` | `"DEBUG"` | ファイル sink/backend の最低 level |
| `logging.console_level` | `str` | `"INFO"` | CLI/コンソール向け最低 level |
| `logging.gui_level` | `str` | `"INFO"` | GUI ログペインの初期表示 level。設定画面には出さず、パネルのチェックボックスで一時変更する |
| `logging.structured_enabled` | `bool` | `True` | JSON Lines の構造化ログを出すか |
| `logging.human_log_enabled` | `bool` | `True` | 人間向け text ログを出すか |
| `logging.file_max_bytes` | `int` | `10485760` | 人間向けログ、framework JSONL、実行単位 JSONL の rotation サイズ |
| `logging.file_backup_count` | `int` | `3` | ローテーション世代数 |
| `logging.file_retention_days` | `int` | `14` | 人間向けログと framework JSONL の保持日数 |
...
| ユニット | `test_technical_log_includes_framework_error_context` | `FrameworkError` を渡した `TechnicalLog.extra` に `error_code` と `error_details` が入る |
