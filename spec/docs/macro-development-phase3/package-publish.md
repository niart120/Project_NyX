...
`pyproject.toml` の配布名を `nyxfw` に寄せる。publish は GitHub Actions trusted publishing を第一候補とし、PyPI API token やローカル `.pypirc` に依存しない。初回 publish 前に PyPI / TestPyPI 側で pending publisher を登録する。
