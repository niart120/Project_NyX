...
> **対象領域**: `README.md`, `docs\macro-development\`, `src\nyxpy\framework\`, `examples\`, `.agents\skills\`, `spec\framework\rearchitecture\`, `spec\macro\`
...
`uv add` はマクロ実装側が `nyxpy.framework.*` を import するライブラリ依存として自然であり、`uv tool install` は `nyx-cli` / `nyx-gui` をツールとして使う導入に向く。PyPI 配布名は `nyxpy-fw`、import 名は `nyxpy` とする。`nyxpy-fw` は import package `nyxpy` と framework 用途の関係を示し、既存 PyPI の `nyxpy` との衝突を避けるために採用する。
...
|----------|------------------------------|--------------------------|----------|
| `uv tool install nyxpy-fw` | uv tool の隔離環境 | NyX と NyX の依存関係。マクロ固有の追加依存は別途 tool 環境へ入れる必要がある | 既存マクロを実行する利用者 |
| `uv add nyxpy-fw` | マクロ実装プロジェクトの仮想環境 | そのプロジェクトの依存関係すべて | マクロを開発し、追加ライブラリやテストを同じ環境で扱う実装者 |
...
| マクロ生成 skill | AI agent | 新規マクロ作成時の調査、雛形生成、テスト追加を手順化 | `.agents\skills\` |
...
| CLI | ツール導入後にテンプレートや docs の場所を表示する | `uv tool install nyxpy-fw` で使える `nyx-cli` から scaffold / docs URL を出す |
...
| 文書 | 目的 | 記述内容 |
|------|------|----------|
| `.agents\skills\macro-development\SKILL.md` | agent に新規マクロ作成・更新の手順を実行させる | 調査順、配置、雛形、テスト、禁止事項、完了条件 |
...
clone しないマクロ開発者には、このリポジトリの `.agents\skills\macro-development\SKILL.md` を取得して各自の agent 環境へ導入する手順を案内する。skill はこのリポジトリで保守し、GitHub Pages 上の `agent-brief.md` から取得先へ誘導する。現時点では skill を package data として同梱しない。
...
| `docs\macro-development\macro-layout.md` | `macros\`, `resources\` の使い分けと `examples\` が利用者の作業場所ではないこと | `README.md`, `MACRO_MIGRATION_GUIDE.md` |
...
| `docs\macro-development\testing.md` | 単体テスト、実機テスト、`@pytest.mark.realdevice`、通常検証と参考実装検証の分離 | `pyproject.toml`, `tests\`, `examples\tests` |
...
| 個別マクロ仕様 | `spec\macro\<macro_id>\spec.md` | 実装方針の根拠として必要に応じて参照する |
| 参考実装 | `examples\macros\`, `examples\resources\`, `examples\tests\` | フレームワーク開発者が管理する参考実装として扱う。マクロ開発者の配置先・共有導線にはしない |
...
- 実装者のマクロ本体は `macros\<macro_id>`、設定・画像資材は `resources\<macro_id>` に置く。
- `examples\` はフレームワーク開発者が管理する参考実装であり、マクロ開発者の配置先・共有導線として案内しない。
...
| 作業 | 成果物 |
|------|--------|
| PyPI 配布名と import 名を決める | 配布名 `nyxpy-fw` / import 名 `nyxpy` |
...
| `examples\macros` / `examples\resources` / `examples\tests` の現行構成を確認する | examples を利用者の作業場所として案内しないための境界整理 |
...
現行コードでは、配布名は `pyproject.toml` の `project.name = "nyxpy-fw"`、import 名は `src\nyxpy\` の `nyxpy` である。`nyxpy` は PyPI 上で既存プロジェクトが使用しているため配布名には使わない。`project-nyx` は未登録で旧設定との整合はあったが、公開パッケージ名としてはリポジトリ内向きに見えるため採用しない。`nyxpy-fw` は import package `nyxpy` と framework 用途の関係を示せるため採用する。
...
| README の現行内容 | 移設先 |
|-------------------|--------|
| `macros\` / `resources\` はローカル作業用、`examples\` はフレームワーク開発者が管理する参考実装 | `docs\macro-development\macro-layout.md` |
...
| 契約 | docs で扱う粒度 |
|------|-----------------|
| マクロ配置 | 実装者は `macros\<macro_id>` と `resources\<macro_id>` を使う。`examples\` はフレームワーク開発者が管理する参考実装であり、配置先・共有導線ではない。 |
...
| testing | 通常の `pytest` は `tests`, `macros` を収集し、実機テストは `@pytest.mark.realdevice` で分離する。参考実装のテストは `examples\tests` を明示して実行する。 |

##### examples の境界整理

`examples\` はフレームワーク開発者が管理する参考実装・検証対象である。マクロ開発者向け docs では `examples\` 配下への移動、共有、依存を通常導線として案内しない。参考実装のテストをメンテナが確認する場合だけ、`examples\tests` を明示して実行する。
...
| `.agents\skills\macro-development\SKILL.md` を作る | AI agent によるマクロ生成・更新手順 |
...
| examples が利用者の作業場所ではないことを明確化する | `macro-layout.md`, `testing.md` |
...
| PyPI 公開に必要な package metadata / publish 手順を整備する | `nyxpy-fw` 配布準備と publish 手順 |
...
- `macros\` / `resources\` が実装者の配置先であり、`examples\` はフレームワーク開発者が管理する参考実装であることが明記されている。
