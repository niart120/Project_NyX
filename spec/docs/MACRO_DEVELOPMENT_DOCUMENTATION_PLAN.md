...
| `nyx-cli` または `python -m nyxpy` から docs URL / scaffold 生成へ到達する導線を検討する | CLI 導線案 |
...
| scaffold を CLI に入れるか | `python -m nyxpy macro new` / `python -m nyxpy init --macro` / `nyx-cli scaffold` / なし | 生成サービスを先に作り、入口は再検討 |
