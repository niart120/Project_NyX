...
| package metadata | `uv add nyxfw`, PyPI project page, dependency resolver | `[project]` を `nyxfw` 配布に合わせ、manual publish workflow を用意する |
| CLI | `uv tool install nyxfw` 後の最初の入口 | `nyxpy ...` を主導線にし、`nyx-cli` / `nyx-gui` は alias として扱う |
...
`pyproject.toml` の `[project]` は `name = "nyxfw"` に更新済みであり、import package 名 `nyxpy` と分離している。publish workflow は manual dispatch の trusted publishing 準備として配置する。
...
| 8 | package-publish | すべての package contents と docs 導線が揃った後、`nyxfw` として metadata / build / publish 手順を確定する | `uv build`, `uvx twine check --strict dist\*`, wheel contents 確認が通る |
...
- [x] `nyxfw` 向け package metadata を整備する
