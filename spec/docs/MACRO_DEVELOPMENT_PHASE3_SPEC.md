...
| CLI | `uv tool install nyxfw` 後の最初の入口 | `nyx-cli docs` と scaffold 入口を分けて検討する |
...
後方互換性はこの repository のアルファ版ポリシーでは必須ではない。旧形式を残す場合は互換 shim ではなく、現行 UX として必要な理由を明記する。scaffold の入口は上記を第一候補にするが、生成サービスを先に作り、実装時に `python -m nyxpy init --macro <macro_id>` などへ変更してよい。
...
scaffold は `macro_id` を受け取って `macros\<macro_id>` と `resources\<macro_id>` を作る生成サービスとして先に設計する。CLI 入口は `nyx-cli scaffold` に固定せず、`python -m nyxpy init --macro <macro_id>`、`python -m nyxpy macro new <macro_id>`、将来の統合 console script のいずれが自然かを実装時に選ぶ。
...
`nyx-cli` はマクロ実行専用で、`macro_name`、`--serial`、`--capture` が必須である。docs URL 表示のようなハードウェア不要操作を追加しにくい。workspace 初期化は `python -m nyxpy init` 側にあり、scaffold 生成はそちらとの接続も候補になる。

#### 判断

CLI 導線を実装する場合は subcommand 構成へ移行する。ただし scaffold は `nyx-cli scaffold` 前提にせず、生成サービスを先に作ってから公開入口を選ぶ。

| command | 役割 | ハードウェア要否 |
|---------|------|------------------|
| `nyx-cli run <macro_name>` | マクロを実行する | 必要 |
| `nyx-cli docs` | 公開 docs URL、agent brief、pydoc 参照方法を表示する | 不要 |
| `python -m nyxpy macro new <macro_id>` | `macros\<macro_id>` と `resources\<macro_id>` に雛形を生成する候補 | 不要 |
| `python -m nyxpy init --macro <macro_id>` | workspace 初期化と同時に雛形を生成する候補 | 不要 |
...
- 採用した scaffold 入口で `sample_turbo` の新規ファイルを生成できる
...
| 6 | CLI 導線 | docs URL と scaffold template が揃ってから、`nyx-cli docs` と scaffold の公開入口を配線する | docs 表示と scaffold 生成がハードウェア引数なしで動く |
...
| CLI scaffold | `python -m nyxpy macro new sample_turbo` | 既定配置へ雛形を生成できること。入口を変更した場合はコマンドも更新する |
...
- [ ] scaffold 生成サービスと公開 CLI 入口を追加する
