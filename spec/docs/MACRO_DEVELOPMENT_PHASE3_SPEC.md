...
`nyx-cli` はマクロ実行専用で、`macro_name`、`--serial`、`--capture` が必須である。docs URL 表示のようなハードウェア不要操作を追加しにくい。workspace 初期化は `python -m nyxpy init` 側にあり、console script としての `nyxpy` は未定義である。
