...
> **前提**: PyPI 配布名は `nyxpy-fw`、import package 名は `nyxpy` とする。公開 docs は GitHub Pages に統一し、GitHub Wiki は使わない。
...
| `spec\docs\macro-development-phase3\package-publish.md` | 新規 | `nyxpy-fw` package metadata と publish 手順 |
...
| package metadata | `uv add nyxpy-fw`, PyPI project page, dependency resolver | `[project]` を `nyxpy-fw` 配布に合わせ、manual publish workflow を用意する |
| CLI | `uv tool install nyxpy-fw` 後の最初の入口 | `nyxpy ...` を主導線にし、`nyx-cli` / `nyx-gui` は alias として扱う |
...
現行 `nyx-cli` は positional `macro_name` と `--serial` / `--capture` を要求するため、docs 表示や scaffold のようなハードウェア不要操作と相性が悪い。Phase 3 では `nyxpy` console script を主導線として追加し、`run`、`docs`、`init`、`create`、`gui` を分離する。
...
`nyx-cli` は `nyxpy run`、`nyx-gui` は `nyxpy gui` の alias として扱う。後方互換性はこの repository のアルファ版ポリシーでは必須ではない。旧形式を残す場合は互換 shim ではなく、現行 UX として必要な理由を明記する。
...
`src\nyxpy\py.typed` は追加済みである。公開 API には型ヒントがあり、インストール済み package を型検査器が typed package として扱える。
...
`pyproject.toml` の `[project]` は `name = "nyxpy-fw"` に更新済みであり、import package 名 `nyxpy` と分離している。publish workflow は manual dispatch の trusted publishing 準備として配置する。

#### 判断

Phase 3 では `nyxpy-fw` として公開できる metadata を整える。実際の publish は、build artifact と手順の確認後に別判断とする。
...
| workflow | `.github\workflows\publish.yml` を manual dispatch のみで配置し、TestPyPI と PyPI を input で選ぶ |
...
PyPI 本番 target は `v*` tag からの manual dispatch だけを許可する。TestPyPI は tag なしでも手動検証できる。

#### 完了条件

- `uv build` が成功する
- `uvx twine check --strict dist\*` が成功する
- wheel metadata の Name が `nyxpy-fw` になっている
- `nyxpy`, `nyx-cli`, `nyx-gui` の console script が wheel metadata に含まれる

### 4.3 scaffold 方針

#### 現状

`docs\macro-development\macro-template.md` には雛形説明があるが、package からファイルとして生成する導線はない。`ensure_workspace()` は `.nyxpy`, `macros`, `resources`, `snapshots`, `runs`, `logs` と `macros\__init__.py` を作るため、workspace 初期化と macro 個別生成の責務は分ける必要がある。

#### 判断

Phase 3 では、最小テンプレートを package data として同梱する方針を採る。`examples\macros` は参照用サンプルであり、scaffold の生成元にはしない。

scaffold は `macro_id` を受け取って `macros\<macro_id>` と `resources\<macro_id>` を作る生成サービスとして設計する。公開入口は `nyxpy create <macro_id>` とする。`nyxpy new` は将来の生成対象が増えた場合に多義的になるため使わない。

`nyxpy init` は通常、workspace 初期化後にサンプルマクロ `sample_macro` も生成する。空 workspace だけが必要な場合は `nyxpy init --blank` を使う。`nyxpy create <macro_id>` は workspace が存在しない場合に失敗し、先に `nyxpy init` を実行するよう案内する。
...
- `ensure_workspace()` が作る共通ディレクトリと、scaffold が作る macro 個別ディレクトリの責務が分かれている
...
`nyxpy` console script は `init`, `create`, `run`, `docs`, `gui` を持つ。`nyx-cli` は `nyxpy run` の alias として、`macro_name`、`--serial`、`--capture` を要求する実行専用入口にしている。

#### 判断

`nyxpy` console script を追加し、`python -m nyxpy` と同じ parser を使う。`nyx-cli` と `nyx-gui` は alias として残す。

| command | 役割 | ハードウェア要否 |
|---------|------|------------------|
| `nyxpy init` | workspace とサンプルマクロを作成する | 不要 |
| `nyxpy init --blank` | サンプルなしの空 workspace を作成する | 不要 |
| `nyxpy create <macro_id>` | `macros\<macro_id>` と `resources\<macro_id>` に雛形を生成する | 不要 |
| `nyxpy run <macro_name>` | マクロを実行する | 必要 |
| `nyxpy docs` | 公開 docs URL、agent brief、pydoc 参照方法を表示する | 不要 |
| `nyxpy gui` | GUI を起動する | 不要 |
| `nyx-cli` | `nyxpy run` の alias | 必要 |
| `nyx-gui` | `nyxpy gui` の alias | 不要 |

#### `docs` 出力内容

`nyxpy docs` はブラウザを自動起動しない。CLI では URL とローカル参照方法を標準出力へ出す。
...
- `nyxpy docs` が serial / capture なしで成功する
- `nyxpy create sample_turbo` が新規ファイルを生成できる
- `nyxpy run sample_turbo --serial COM3 --capture "Capture Device"` で実行機能に到達できる
- `nyx-cli` alias から `nyxpy run` 相当の実行機能に到達できる

### 4.5 MkDocs + mkdocstrings 検証方針

#### 現状

`docs\macro-development\` は Markdown として存在し、`mkdocs.yml` と `docs\api\framework.md` で API reference 生成構成を追加済みである。API reference は手書き Markdown ではなく docstring / 型ヒントから生成する方針である。

#### 判断

MkDocs + mkdocstrings を採用する。Markdown 中心の既存 docs と相性がよく、API reference だけを docstring から差し込めるためである。
...
| `nyxpy.framework.core.macro.exceptions` | マクロ実装者が読む framework error 階層 |
| `nyxpy.framework.core.io.ports` | `FrameNotReadyError` など入出力例外 |
...
- `mkdocs.yml` に GitHub Pages 用の `site_url` が設定されている

### 4.6 GitHub Pages 配信方針

#### 現状

`.github\workflows\test.yml` は Python CI を担当する。docs build / deploy は `.github\workflows\docs.yml` で分離して実行する。

#### 判断

GitHub Pages へ生成 docs を配信する workflow を追加する。公開先は repository Pages とし、Wiki は使わない。公開対象はマクロ開発者向け docs と API reference だけに限定せず、通常利用者向けの `docs\user-guide\` も同じ site に含める。

#### workflow 方針

| 項目 | 方針 |
|------|------|
| trigger | `workflow_dispatch`, `master` push, `master` 向け pull request |
| build | `uv sync --locked --only-group docs --no-install-project` 後に `uv run --no-sync mkdocs build --strict` |
| deploy | `actions/configure-pages`, `actions/upload-pages-artifact`, `actions/deploy-pages` を使う |
| pull request | deploy せず build のみ確認する |
| permission | workflow default は `contents: read`。`pages: write`, `id-token: write` を deploy job に限定する |
| environment | deploy job に `github-pages` environment と deployment URL を設定する |
| concurrency | Pages deploy は `group: pages`, `cancel-in-progress: false` とする |

#### 完了条件

- pull request で docs build が検証される
- `master` push または manual dispatch で Pages artifact を deploy できる
- 公開後に `https://niart120.github.io/Project_NyX/`, `/macro-development/`, `/api/framework/` を表示できる
- `docs\user-guide\` 追加後に通常利用者向けページも同じ Pages site で公開できる
- README / `nyxpy docs` から公開 URL へ到達できる
...
| 4 | GitHub Pages 配信 | `nyxpy docs` と README から参照する公開 URL を確定する | pull request で docs build、`master` / manual dispatch で deploy 可能 |
| 5 | scaffold template | macro 個別生成サービスの入力になる package data を先に作る。`examples\` を生成元にしない方針を実装へ落とす | template から `macros\<macro_id>` / `resources\<macro_id>` の標準配置を生成できる |
| 6 | CLI 導線 | docs URL と scaffold template が揃ってから、`nyxpy` の subcommand と alias を配線する | docs 表示と scaffold 生成がハードウェア引数なしで動く |
...
| 8 | package-publish | すべての package contents と docs 導線が揃った後、`nyxpy-fw` として metadata / build / publish 手順を確定する | `uv build`, `uvx twine check --strict dist\*`, wheel contents 確認が通る |
...
| type check gate | `uv run ty check src\nyxpy --output-format concise --no-progress` | PEP 561 公開後に利用者へ見える型ヒントを CI gate として検査する |
| package build | `uv build` | sdist / wheel を作成できること |
| metadata check | `uvx twine check --strict dist\*` | PyPI metadata が妥当であること |
...
| docs build | `uv sync --locked --only-group docs --no-install-project; uv run --no-sync mkdocs build --strict` | Markdown docs と API reference を生成できること |
| CLI docs | `nyxpy docs` | ハードウェア引数なしで docs URL を表示できること |
| CLI scaffold | `nyxpy init --blank; nyxpy create sample_turbo` | 既定配置へ雛形を生成できること |
...
- [x] GitHub Pages workflow を追加する
- [x] scaffold template を package data として追加する
- [x] `nyxpy docs` の導線を追加する
- [x] `nyxpy create` の導線を追加する
- [x] `nyx-cli` / `nyx-gui` alias を `nyxpy run` / `nyxpy gui` に接続する
- [x] `docs\macro-development\README.md` から公開 docs / API reference / scaffold 導線へリンクする
- [x] `nyxpy-fw` 向け package metadata を整備する
- [x] package build と metadata check を通す
