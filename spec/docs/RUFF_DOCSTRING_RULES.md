...
Ruff formatter の docstring 対応は、`format.docstring-code-format` による docstring 内コード例の整形に限られる。この設定は既定で `false` であり、Google style の section header、section 前後の空行、空 section は formatter では整形しない。`D405` から `D414` までは lint 側で扱うルールであり、自動修正が必要な場合は `ruff check --fix` の対象として判断する。
