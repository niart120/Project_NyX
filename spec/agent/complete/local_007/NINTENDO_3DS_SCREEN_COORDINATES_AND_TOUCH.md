...
| タッチパネルトグル | GUI の「コントローラー」見出し横に置く `タッチパネル` checkbox。プレビュー上の下画面タッチ送信をユーザーが明示的に有効化・無効化する |
...
既存の 3DS シリアルプロトコル対応では `Command.touch_down(x, y)` が存在するが、キャプチャ画像上の座標と 3DS 下画面の実タッチ座標を変換する共通 API がない。`Command.capture()` は `1280x720` のアスペクトボックス画像を返す想定であり、3DS の `400x480` 画面は `600x720` に拡大されて左右 `340px` の黒帯付きで配置される。3DS の下画面には本体由来の左右ピラーボックスもあるため、HD キャプチャ上の下画面実領域は `x=400, y=360, width=480, height=360` になる。この補正を各マクロ・GUI 実装で重複させると、クリック位置とタッチ座標がずれやすい。加えて、プレビューをクリックしただけでタッチ入力が送信されると誤操作になりやすいため、ユーザーが GUI でタッチパネル送信を明示的に有効化できる必要がある。
...
| タッチパネル送信の誤操作防止 | プレビュークリックは常に送信される | `タッチパネル` checkbox が有効な場合だけ送信 |
| タッチパネル有効状態の保存 | なし | `gui.preview_touch_enabled` に保存し、初期値は `False` |
...
- プレビュータッチ送信は `タッチパネル` checkbox がチェック済みの場合だけ行う。
- `タッチパネル` checkbox はユーザー設定 `gui.preview_touch_enabled` へ永続化し、初期値は未チェックとする。
- 3DS 以外の protocol でも `タッチパネル` checkbox は操作可能なままにし、checkbox 切替時の非対応メッセージは表示しない。
...
| `src\nyxpy\framework\core\settings\global_settings.py` | 変更 | `gui.preview_touch_enabled` を設定 schema に追加する |
...
| `src\nyxpy\gui\main_window.py` | 変更 | 「コントローラー」見出し横に `タッチパネル` checkbox を追加し、`PreviewPane` のタッチ signal を gate する |
...
| `tests\unit\framework\settings\test_settings_schema.py` | 変更 | `gui.preview_touch_enabled` の既定値と型検証を追加する |
...
| `tests\gui\test_main_window.py` | 変更 | タッチパネル checkbox、プレビュー下画面操作、非対応 protocol 時の no-op とステータス表示を検証する |
...
タッチパネルトグルは protocol capability と独立したユーザー設定である。3DS 以外でも checkbox は有効状態のまま操作でき、切替時には非対応メッセージを出さない。プレビュータッチ送信時の gate は `checkbox checked` かつ `supports_touch` の両方を満たす場合だけ通過する。
...
MainWindow                       -> GlobalSettings + VirtualControllerModel.supports_touch_input()
...
`MainWindow` は `タッチパネル` checkbox の状態を `GlobalSettings` に保存し、preview touch signal を受けた時点で checkbox 状態と controller capability を確認する。`PreviewPane` は checkbox 状態を知らない。
...
| checkbox 切替 | シリアル送信を発生させない |

### 並行性・スレッド安全性

座標変換関数は不変 dataclass と純粋関数で構成し、スレッド共有状態を持たない。GUI の mouse event と checkbox 切替は Qt の UI thread 上で処理する。`VirtualControllerModel` は既存のボタン操作と同じく GUI lifetime の `ControllerOutputPort` を利用し、送信エラーは logger へ記録して再送出する。
...
`MainWindow` は preview touch signal を直接 `VirtualControllerModel.touch_*` へ接続しない。`_handle_preview_touch_down()` は最初に `タッチパネル` checkbox の状態を確認し、未チェックなら controller へ送信せず、ステータス表示も変更しない。チェック済みの場合だけ `virtual_controller.model.supports_touch_input()` を確認し、非対応なら controller へ送信せず、クリックごとにステータスバーへ「現在のプロトコルは 3DS タッチ入力に対応していません」を表示する。非対応状態で drag / release signal が届いた場合も送信しない。3DS protocol 選択中かつ checkbox がチェック済みの場合だけ `touch_down`、座標変化を伴う `touch_move`、`touch_up` を controller port へ渡す。

`タッチパネル` checkbox は `_VirtualControllerPanel` の見出し行に置く。左側に `コントローラー` label、右側に `タッチパネル` checkbox を配置し、既存の仮想コントローラキャンバスの body size を変えない。`MainWindow` 初期化時は `global_settings.get("gui.preview_touch_enabled", False)` を checkbox に反映する。checkbox 切替時は `global_settings.set("gui.preview_touch_enabled", checked)` を呼び、ステータスバーやログには出さない。
...
| パラメータ | 型 | デフォルト | 説明 |
|------------|-----|-----------|------|
| `gui.preview_touch_enabled` | `bool` | `False` | GUI プレビュー上の下画面タッチ送信を有効化するか。`タッチパネル` checkbox と同期する |
...
| GUI | `test_main_window_initializes_touch_panel_checkbox_from_settings` | `gui.preview_touch_enabled` の保存値が `タッチパネル` checkbox 初期状態に反映される |
| GUI | `test_main_window_saves_touch_panel_checkbox_to_settings` | checkbox 切替で `gui.preview_touch_enabled` が保存され、ステータス表示は変更されない |
| GUI | `test_main_window_wires_preview_touch_to_virtual_controller_model` | MainWindow が preview touch signal と model を接続する |
| GUI | `test_main_window_ignores_preview_touch_when_touch_panel_is_disabled` | touch 対応 controller でも checkbox 未チェックなら preview touch を送信せず、ステータス表示も変更しない |
...
| GUI | `test_main_window_keeps_touch_panel_checkbox_enabled_for_non_touch_protocol` | touch 非対応 controller でも checkbox は操作可能なまま維持される |
