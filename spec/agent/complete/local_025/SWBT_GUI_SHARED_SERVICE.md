...
| GUI | `test_apply_settings_defers_builder_swap_during_run` | macro 実行中の設定反映を deferred にする |
...
- capture 変更と controller 変更の反映粒度を分け、変更していない lifetime port は builder 入れ替え時に新 builder へ移す。移した port に対応する旧 builder の factory shutdown callback は実行しない。
...
uv run pytest tests\unit\framework\runtime\test_runtime_builder.py -m "not realdevice and not swbt"
uv run pytest tests\gui -m "not realdevice and not swbt"
```
