...
左列の高さは中央 widget の利用可能高に合わせる。仮想コントローラは左列下端に固定し、マクロ一覧パネルを伸縮領域として余剰高さを吸収する。中央列ではプレビュー固定サイズを維持し、プレビュー下ツールログが縦方向の余剰高さを吸収する。右マクロログは中央 widget の利用可能高まで伸びる。これにより、固定サイズ pane が上下中央に寄って不自然な余白を作らず、仮想コントローラは状態バー側に接地し、プレビュー下ツールログは仮想コントローラ下へ広がらない。中央 widget の下端 margin は `0` とし、ステータスバー直上に別フッターのような空白を作らない。

### 6.4 UI フォント方針

UI パーツは役割ごとにフォントを分ける。初期実装では OS / Qt の既定フォントを基準にし、サイズを絶対指定しすぎない。
...
| controller button | `A/B/X/Y`, `L3/R3`, `ZL/ZR` 等 | bold | HD / FullHD は従来より 1px 上げる |
| log view | ツールログ / マクロログ本文 | regular | 等幅フォント |
| status bar | 接続状態 / 実行状態 | regular | Qt 既定サイズ |

太字は pane title とコントローラー内ボタンに限定する。操作ボタンまで太字にすると押下可能領域の主張が強くなり、ログや一覧の視認性を邪魔するため使わない。ログ pane の外側幅は対応する列幅と一致させ、ログ本文 widget は pane 内の余白で狭めない。ツールログ本文はプレビューと同じ幅に見えることを優先する。
