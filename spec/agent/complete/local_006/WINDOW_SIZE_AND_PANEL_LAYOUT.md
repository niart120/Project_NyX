...
左列の高さは中央 widget の利用可能高に合わせる。マクロ一覧パネルの高さはプレビュー高さに合わせ、仮想コントローラ見出しとツールログ見出しの上端を揃える。中央列ではプレビュー固定サイズを維持し、プレビュー下ツールログが縦方向の余剰高さを吸収する。右マクロログは中央 widget の利用可能高まで伸びる。これにより、固定サイズ pane が上下中央に寄って不自然な余白を作らない。中央 widget の上下 margin は `0` とし、メニューバー直下とステータスバー直上に別フッターのような空白を作らない。
...
| controller button | `A/B/X/Y`, `L3/R3`, `ZL/ZR` 等 | bold | HD / FullHD は従来より 1 point 上げる |
