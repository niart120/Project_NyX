...
左列の高さは中央 widget の利用可能高に合わせる。仮想コントローラは左列下端に固定し、マクロ一覧パネルを伸縮領域として余剰高さを吸収する。中央列ではプレビュー固定サイズを維持し、プレビュー下ツールログが縦方向の余剰高さを吸収する。右マクロログは中央 widget の利用可能高まで伸びる。これにより、固定サイズ pane が上下中央に寄って不自然な余白を作らず、仮想コントローラは状態バー側に接地し、プレビュー下ツールログは仮想コントローラ下へ広がらない。
