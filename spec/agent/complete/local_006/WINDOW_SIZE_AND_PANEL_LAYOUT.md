...
左列の高さは中央列の `preview_height + gap + preview_tool_log_height` に合わせる。仮想コントローラは左列下端に固定し、マクロ一覧パネルを伸縮領域として余剰高さを吸収する。これにより、仮想コントローラは状態バー側に接地し、プレビュー下ツールログは仮想コントローラ下へ広がらない。
