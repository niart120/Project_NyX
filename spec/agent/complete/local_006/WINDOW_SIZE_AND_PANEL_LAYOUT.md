...
コントローラー内ボタンの文字サイズは `px` ではなく `QFont` の point size で指定する。Qt の高 DPI 対応に任せ、Windows の表示スケールや OS 側フォント設定の影響を受けられるようにする。太字は pane title とコントローラー内ボタンに限定する。操作ボタンまで太字にすると押下可能領域の主張が強くなり、ログや一覧の視認性を邪魔するため使わない。ログ pane の外側幅は対応する列幅と一致させ、ログ本文 widget は pane 内の余白で狭めない。ツールログ本文はプレビューと同じ幅に見えることを優先する。
