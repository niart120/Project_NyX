...
`ponkan-python` の最低バージョンを `0.2.0` に更新し、上流の `list_capture_devices()` と structured `CaptureError` を NyX 側の framework API として利用できるようにする。GUI の通常メニュー描画では引き続き `ponkan` 本体を import せず、実機 listing は明示的な診断操作として分離する。
...
| capture profile | `ponkan` が公開する capture target metadata。`0.2.0` 時点の実装済み profile は `n3dsxl` |
...
| 古い上流 API | `AttributeError` の可能性 | `NYX_PONKAN_CAPTURE_API_UNSUPPORTED` として扱う |
...
| `tests/unit/framework/hardware/test_ponkan_capture.py` | 変更 | profile metadata 利用、unsupported API、structured error details を検証する |
...
Project NyX の framework はアルファ版であり、互換 shim は追加しない。今回の変更では最低版を `ponkan-python 0.2.0` へ上げ、古い `0.1.x` API をサポート対象から外す。古い API が import 環境に残っている場合は `AttributeError` ではなく `ConfigurationError` の `NYX_PONKAN_CAPTURE_API_UNSUPPORTED` として報告するが、これは互換維持ではなく設定エラーの明確化である。

既存の `capture_source_type="capture"`、`capture_provider="ponkan"`、`capture_device_profile="n3dsxl"`、`ponkan_*` settings は削除しない。これは後方互換性のためだけではなく、現行の runtime 設定 surface として `PonkanCaptureSourceConfig` がまだ利用しているためである。将来、上流 profile API に完全に寄せて低レベル queue / timeout 設定を NyX から隠す場合は別仕様で破壊的変更として扱う。
...
今回の実装は「壊さないために旧 API を温存する」方向には寄せない。最低版を 0.2.0 へ上げ、古い上流 API は unsupported として扱う。一方で、provider/profile/advanced settings の全面再設計は行わない。これは互換性への遠慮ではなく、GUI 表示・runtime config・実機接続範囲が local_017 / local_019 で既に分割されており、今回の目的が listing API と structured error の取り込みに限定されるためである。
...
`lister` はテスト注入用である。production では `import_module("ponkan")` から `list_capture_devices` を取得する。`ponkan` import 失敗、`list_capture_devices` 不在、上流呼び出し失敗は snapshot として返し、生の例外を GUI/CLI に漏らさない。
...
`_open_ponkan_capture()` は `get_capture_profile(config.device_profile)` を呼び、profile の `model` を `CaptureConfig(source=..., model=...)` に渡す。これにより、NyX 側の profile id と上流の model string の対応を上流 metadata から得る。
...
`CaptureConfig.for_profile()` や `open_capture(profile=...)` は `ponkan-python 0.2.0` には存在しないため使わない。
...
| `capture_device_profile` | `str` | `"n3dsxl"` | `get_capture_profile()` に渡す profile id |
...
| `list_capture_devices` missing | `PonkanCaptureDiscoverySnapshot(reason="missing_package", remediation="Install ponkan-python 0.2.0 or later.")` |
...
| 古い上流 API | `NYX_PONKAN_CAPTURE_API_UNSUPPORTED` |
...
| ユニット | `test_ponkan_open_capture_uses_upstream_default_device_selection` | `get_capture_profile()` の model を `CaptureConfig` に渡し、物理個体 selector は渡さない |
| ユニット | `test_ponkan_capture_unsupported_api_is_configuration_error` | `ponkan-python 0.2.0` API 不在を `ConfigurationError` に変換する |
...
`pytest` は `629 passed, 9 skipped` である。
...
- [x] 古い上流 API を `NYX_PONKAN_CAPTURE_API_UNSUPPORTED` として扱う
- [x] ユニットテストを追加
