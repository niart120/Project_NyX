...
> **関連ドキュメント**: `../../../framework/archive/protocol_design.md`, `../../../framework/archive/hardware_design.md`, `FOLLOWUP_FIXES.md`, `../local_009/NINTENDO_3DS_STICKPAD_NEW_FIRMWARE.md`
> **参考資料**: ユーザ提供の通信仕様スクリーンショット、<https://www.3dscontroller.com/>
> **破壊的変更**: あり。`local_009` でスライドパッドの軸値を New Firmware StickPad DAC 仕様へ更新した。

## 1. 概要

### 1.1 目的

Nintendo 3DS 向けシリアル通信デバイスのボタン、スライドパッド、C スティック、タッチ、スリープ抑止コマンドをフレームワーク層で生成できるようにする。スライドパッドは New Firmware の StickPad DAC 仕様、C スティックは符号付き 8 bit オフセット仕様で生成する。
...
| スライドパッド | 3DS の左アナログ入力。NyX では `LStick` から New Firmware の StickPad DAC 仕様へ変換する |
| C スティック | New 3DS 系の右アナログ入力。NyX では `RStick` から変換する |
| タッチ座標 | 下画面のタッチ位置。X は `0..319`、Y は `0..239` の整数で指定する |
...
| 3DS スライドパッド軸値の互換性 | 旧 DAC 範囲 (`RIGHT/DOWN=FA`, `LEFT/UP=7E`) | New Firmware StickPad DAC (`CENTER=80 80`, `LEFT/UP=FF`, `RIGHT/DOWN=00`) |
...
`local_009` の変更により、3DS プロトコルにおける `LStick` の生成バイト列は破壊的に変更される。旧 DAC 範囲 (`RIGHT/DOWN=FA`, `LEFT/UP=7E`) は残さず、New Firmware の StickPad DAC 仕様 (`CENTER=80 80`, `LEFT/UP=FF`, `RIGHT/DOWN=00`) を使う。`RStick` は従来どおり C スティック仕様 (`CENTER=00 00`, `RIGHT/DOWN=7F`) を使う。`Button.MINUS` は 3DS の SELECT、`Button.PLUS` は START に変換し、Switch 向けマクロで使われる既存定数を最大限再利用する。`Button.CAP` / `Button.LS` / `Button.RS` は 3DS の S2/T3 仕様に対応する入力がないため、3DS プロトコルでは `UnsupportedKeyError` を送出する。
...
| 3DS `LStick` / `RStick` の軸値変換 | 各スティックの仕様に従う |
...
`build_press_command` / `build_hold_command` / `build_release_command` / `build_touch_down_command` / `build_touch_up_command` は、常にこの 14 バイトを返す。例: `build_press_command((Button.A, Hat.UP, LStick.RIGHT, RStick.UP, TouchState.down(319, 239)))` は `A1 18 00 A2 00 80 A4 00 80 B2 01 01 3F EF` を返す。タッチ押下中にボタンやスティックを変更した場合は、最後の `B2` サブパケットに現在のタッチ状態を維持する。
...
スライドパッドは `A2 x y` の 3 バイトで送信する。既存の `LStick` は `x` / `y` が `0..255` であるため、New Firmware の StickPad DAC 値へ変換する。NyX 軸値 `0` は `FF`、`128` は `80`、`255` は `00` に対応する。
...
```python
def convert_slide_pad_axis(value: int) -> int:
...
C スティックは `A4 x y` の 3 バイトで送信する。既存の `RStick` を符号付き 8 bit オフセットへ変換する。`127` と `128` は中央のデッドゾーンとして `00` に変換する。
...
タッチは 5 バイトで送信する。X は `0..319`、Y は `0..239` の範囲だけ許可する。3DS のタッチ画面は横幅 320 であり、X 座標は 1 バイト (`0..255`) に収まらないため 16 bit 相当の `x_high` / `x_low` に分割する。Y 座標は `0..239` で 1 バイトに収まるため `y_low` のみを送信する。
...
| フィールド | 定義 | 例 (`x=319`, `y=239`) |
|------------|------|-----------------------|
| `x_high` | `x` の上位 8 bit。`(x >> 8) & 0xFF` | `0x01` |
| `x_low` | `x` の下位 8 bit。`x & 0xFF` | `0x3F` |
| `y_low` | `y` の下位 8 bit。`y & 0xFF`。Y は `0..239` のため上位バイトを持たない | `0xEF` |
...
| `touch_x_max` | `int` | `319` | タッチ X の最大値。初回実装では定数として扱い、設定永続化はしない |
| `touch_y_min` | `int` | `0` | タッチ Y の最小値。初回実装では定数として扱い、設定永続化はしない |
| `touch_y_max` | `int` | `239` | タッチ Y の最大値。初回実装では定数として扱い、設定永続化はしない |
...
| ユニット | `test_3ds_slide_pad_presets_use_new_firmware_dac_spec` | `LStick.CENTER` / `LEFT` / `RIGHT` / `UP` / `DOWN` が New Firmware StickPad DAC 仕様と一致する |
...
| ユニット | `test_stick_center_uses_protocol_neutral_axis` | `LStick.CENTER` / `RStick.CENTER` が `(128, 128)` になる |
| GUI | `test_virtual_controller_left_stick_uses_3ds_new_firmware_dac_spec` | 仮想コントローラー左スティックが New Firmware StickPad DAC 仕様の `A2` 軸値を送る |
| GUI | `test_virtual_controller_right_stick_uses_3ds_c_stick_axis_spec` | 仮想コントローラー右スティックが C スティック仕様の `A4` 軸値を送る |
| ユニット | `test_3ds_touch_state_keytype` | `TouchState.down(319, 239)` が末尾に `B2 01 01 3F EF` を含む 14 バイトになる |
| ユニット | `test_3ds_touch_state_release` | `build_release_command((TouchState.down(319, 239),))` がタッチ状態だけを解放する |
| ユニット | `test_3ds_touch_down` | `touch_down(319, 239)` が末尾に `B2 01 01 3F EF` を含む 14 バイトになる |
