...
| FrameTransformer | backend から取得した raw frame を 1280x720 などの target frame へ正規化する共通処理 |
...
WGC backend は raw frame の取得だけを担当する。600x720 などの特殊サイズを 1280x720 キャンバスへ合成する処理は MVP で導入する `FrameTransformer` に委譲し、backend ごとに正規化処理を重複実装しない。
