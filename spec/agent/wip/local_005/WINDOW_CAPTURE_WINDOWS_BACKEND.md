...
| FrameTransformer | backend から取得した raw frame へ必要に応じて 16:9 の黒帯を付与する共通処理 |
...
| `src/nyxpy/framework/core/hardware/window_discovery.py` | 変更 | Windows の window handle、クライアント領域、外枠領域を `WindowInfo` に保持する |
...
WGC backend は raw frame の取得だけを担当する。600x720 などの特殊サイズへ左右または上下の黒帯を追加して 16:9 に整える処理は MVP で導入する `FrameTransformer` に委譲し、backend ごとに黒帯付与処理を重複実装しない。
