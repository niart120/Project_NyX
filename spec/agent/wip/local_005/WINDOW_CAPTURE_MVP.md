...
| FrameTransformConfig | 入力フレームを出力キャンバスへ合成するためのサイズ、リサイズ、オフセット設定 |
| FrameTransformer | BGR frame を 1280x720 などの target frame へ正規化する純粋ロジック |
...
| 出力フレームサイズ | カメラ入力の実質 1280x720 前提 | 既定 1280x720。600x720 などはキャンバス合成で補正 |
...
| `src/nyxpy/framework/core/hardware/frame_transform.py` | 新規 | `FrameTransformConfig` と `FrameTransformer` を実装する |
...
| `tests/unit/hardware/test_frame_transform.py` | 新規 | 1280x720 キャンバス合成、リサイズ、オフセットの単体テスト |
...
| `frame_transform.py` | 入力 frame を target canvas へ合成する純粋ロジック |
...
| frame 正規化 | 600x720 入力を 1280x720 へ合成して 30 FPS を維持 |
...
入力 frame の正規化はキャプチャスレッド内で行い、`latest_frame` には正規化後の BGR frame だけを保存する。これにより `FrameSourcePort` 利用側は入力元の実サイズを意識しない。
...
`FrameTransformer` は以下の規則で frame を 1280x720 などの target canvas へ合成する。

| `resize_policy` | 処理 | offset の扱い |
|-----------------|------|---------------|
| `none` | 入力サイズを維持する | `None` は target 内中央。指定値は左上座標 |
| `fit` | 縦横比を維持して target 内へ収める | resize 後のサイズに対して適用 |
| `stretch` | target 全体へ伸縮する | offset は無視して 0,0 |

600x720 入力を既定の 1280x720 target に `none` で合成する場合、offset 未指定なら x=340, y=0 に配置する。明示的な `capture_offset_x` / `capture_offset_y` がある場合はその値を優先する。貼り付け範囲が target 外へ出た場合は target 内だけをコピーする。背景は黒で固定する。
...
| `ValueError` | `FrameTransformConfig` の target サイズが 0 以下、未知の resize policy |
...
| ユニット | `test_frame_transform_places_600x720_with_explicit_offset` | 600x720 入力を指定オフセットで 1280x720 に配置する |
| ユニット | `test_frame_transform_centers_600x720_when_offset_is_none` | offset 未指定時に x=340, y=0 へ配置する |
| ユニット | `test_frame_transform_fit_keeps_aspect_ratio` | `fit` が縦横比を維持する |
| ユニット | `test_frame_transform_stretch_fills_target` | `stretch` が target 全体へ伸縮する |
...
- [ ] 設定 schema 追加
- [ ] 600x720 入力を 1280x720 へ合成するテスト作成・パス
- [ ] ユニットテスト作成・パス
- [ ] 結合テスト作成・パス
