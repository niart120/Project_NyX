...
> **目的**: メニューバーに「接続」を追加し、キャプチャ入力、コントローラー、プロトコル、FPS、ボーレートを現在の検出結果から切り替えられるようにする
> **関連ドキュメント**: `spec/agent/wip/local_012/DEVICE_CONNECTION_FALLBACK.md`, `spec/agent/complete/local_006/WINDOW_SIZE_AND_PANEL_LAYOUT.md`
...
> **破壊的変更**: あり。`File` menu を廃止し、Settings action は `接続 > 設定...` へ移動する。

## 1. 概要

### 1.1 目的

GUI のメニューバーに「接続」を追加し、現在の実効接続をチェック付きで表示する。接続先、入力ソース、protocol、FPS、baudrate を設定ダイアログを開かずに切り替えられるようにする。
...
| 指標 | 現状 | 目標 |
|------|------|------|
| 接続設定への到達 | File > Settings > 一般 tab | 接続 menu から直接切替 |
...
- `spec/agent/wip/local_012/DEVICE_CONNECTION_FALLBACK.md` の requested / resolved model を採用する。
...
| ファイル | 変更種別 | 変更内容 |
|----------|----------|----------|
| `src/nyxpy/gui/main_window.py` | 変更 | `File` menu を廃止し、`connection_menu.py` へ menu 構築を委譲する。 |
| `src/nyxpy/gui/connection_menu.py` | 新規 | 接続 menu の構築、チェック状態、再読み込み action、設定反映 action を分離する。 |
| `src/nyxpy/gui/app_services.py` | 変更 | menu 用の discovery snapshot、resolved target、接続再読み込み API を提供する。 |
...
破壊的変更あり。`File` menu は廃止し、Settings action は `接続 > 設定...` へ移動する。`表示` menu の window size preset は維持する。設定ファイルの key 名は変更しない。
...
| 設定... | しない | dialog の Apply/OK で保存 | dialog apply 後に既存経路で再評価 | 既存設定 dialog を開く。 |
...
| カメラデバイス | Dummy または検出済み camera | `capture_connection` の resolved target。自動 fallback 中は Dummy にチェック。 |
...
| ボーレート | 現在 protocol の supported baudrate | `serial_baud` と一致する action。非対応値は既定値へ補正してから表示する。 |
...
| テスト種別 | テスト名 | 検証内容 |
|------------|----------|----------|
| GUI | `test_connection_menu_replaces_file_menu` | `File` menu がなく、`接続` menu に `設定...` がある。 |
...
- [ ] `ConnectionMenuSnapshot` と `ConnectionMenuController` の内部 API を確定する。
- [ ] `File` menu を廃止し、`接続 > 設定...` へ Settings action を移動する。
- [ ] `connection_menu.py` を追加し、menu 表示で同期 discovery を呼ばない。
- [ ] キャプチャ入力 menu に入力ソース、カメラデバイス、ウィンドウ、FPS、再読み込みを追加する。
- [ ] コントローラー menu にシリアルデバイス、protocol、baudrate、再読み込みを追加する。
- [ ] Protocol 変更時に baudrate を対応範囲へ補正する。
- [ ] 自動 Dummy fallback と明示 Dummy 選択のチェック状態を分ける。
- [ ] マクロ実行中の接続切替を deferred または disabled として一貫させる。
- [ ] 設定ダイアログの候補列挙を接続 menu と同じ方針へ揃える。
- [ ] GUI テスト作成・パス。
- [ ] `uv run ruff check src\nyxpy tests\gui` を実行する。
