...
| `src/nyxpy/gui/dialogs/settings/device_tab.py` | 変更 | Source 選択に `キャプチャ` を追加し、provider / profile / ponkan backend / timing 設定を表示する |
| `src/nyxpy/gui/app_services.py` | 変更 | capture source 設定変更時に preview / runtime builder を再生成する |
...
`create()` は `PonkanCaptureSourceConfig` を match し、`_create_ponkan_capture_source()` を呼ぶ。capture source は camera discovery を必要としないため、`select_capture_target()` を通らない。`allow_dummy=True` の場合は open / initialize 失敗時に既存 `_FallbackCaptureDevice` で `DummyCaptureDevice` へ fallback する。
...
| 4 | `FrameSourcePortFactory` に capture / ponkan 分岐を追加 | fake factory で reuse / fallback / close を検証 |
| 5 | GUI settings に source 選択を追加 | capture / ponkan / n3dsxl profile 設定保存と preview rebuild を GUI test で確認 |
...
| ユニット | `test_frame_source_factory_falls_back_to_dummy_when_ponkan_open_fails` | `allow_dummy=True` では open 失敗時に Dummy へ fallback する |
| 結合 | `test_runtime_uses_ponkan_capture_frame_source` | fake capture source から `Command.capture()` が `1280x720` BGR frame を返す |
| GUI | `test_device_settings_tab_applies_ponkan_capture_settings` | GUI settings で source / provider / profile / backend / timing を保存する |
...
- [ ] `pyproject.toml` に optional extra `ponkan` を追加
- [ ] capture device 例外 contract を実装
- [ ] `PonkanCaptureSourceConfig` と settings parsing を実装
- [ ] `PonkanCaptureDevice` を fake opener で単体テスト可能に実装
- [ ] `FrameSourcePortFactory` に capture / ponkan source 分岐を追加
- [ ] GUI settings に capture source 設定を追加
