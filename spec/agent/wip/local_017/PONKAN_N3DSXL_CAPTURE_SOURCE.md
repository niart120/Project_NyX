...
| dependency blast radius | 依存なし | `ponkan-python` 本体を optional extra `n3dsxl` に隔離する。PyD3XX は上流の Windows platform-gated 通常依存として解決させる |
...
| ファイル | 変更種別 | 変更内容 |
|----------|----------|----------|
| `pyproject.toml` | 変更 | optional extra `n3dsxl` として `ponkan-python>=0.1.2,<0.2.0 ; sys_platform == "win32"` を追加する |
...
`ponkan-python` は NyX の必須依存にしない。`ponkan-python 0.1.2` では PyD3XX が Windows 向け通常依存になったため、NyX 側では `ponkan-python[d3xx]` を指定しない。NyX の `n3dsxl` extra は、N3DSXL hardware path 全体を通常導入から隔離するために残す。
...
通常の `uv sync` では `ponkan-python` を解決しない。実機開発者は `uv sync --extra n3dsxl` または package install 時の `nyxpy-fw[n3dsxl]` を使う。Windows ではこの extra により `ponkan-python` 経由で PyD3XX も解決される。実装側は `ponkan` package 未導入と、上流が `DependencyUnavailableError` で示す runtime dependency 不足を分けて扱う。
...
def _open_ponkan_capture(config: N3DSXLCaptureSourceConfig) -> PonkanReader:
    ...
    try:
        ...
        raise ConfigurationError(
            "n3dsxl capture dependency is unavailable",
            code="NYX_N3DSXL_CAPTURE_DEPENDENCY_UNAVAILABLE",
            component="N3DSXLCaptureDevice",
...
| `n3dsxl_capture_backend` | `str` | `"auto"` | `auto` / `d3xx` / `d3xx-native`。0.1.2 では `auto` は D3XX 経路 |
...
| 例外クラス | 発生条件 |
|------------|----------|
| `ConfigurationError(code="NYX_N3DSXL_CAPTURE_DEPENDENCY_MISSING")` | `ponkan` package が import できない |
| `ConfigurationError(code="NYX_N3DSXL_CAPTURE_DEPENDENCY_UNAVAILABLE")` | `ponkan` は import できるが、選択 backend の runtime dependency が利用できない |
...
| ユニット | `test_n3dsxl_capture_dependency_unavailable_is_configuration_error` | 上流 `DependencyUnavailableError` を `NYX_N3DSXL_CAPTURE_DEPENDENCY_UNAVAILABLE` に変換する |
...
- [x] `ponkan-python` 本体を NyX の `n3dsxl` optional extra に隔離する方針を確定
