...
| GUI | `test_controller_backend_change_keeps_preview_frame_source` | controller backend 変更だけでは preview を再作成しない |
| GUI | `test_capture_backend_change_keeps_manual_controller` | capture 変更だけでは manual controller を再作成しない |
...
- [ ] adapter refresh が `SwbtAdapterDiscoveryService` だけを呼び、settings を変更しないようにする。
...
- [ ] Disconnect で manual port と factory-managed session を閉じ、controller を `None` に戻す。
- [ ] `VirtualControllerModel.set_controller(...)` が release / close しないことを確認する。
- [ ] macro start 前に manual port を退避してから model から外し、`release()` / `close()` する。
- [ ] macro 実行中は manual input と swbt lifecycle 操作を無効化する。
- [ ] runtime 終了後に自動 reconnect しない。
- [ ] capture backend / capture source と controller backend の変更を独立して反映する。
