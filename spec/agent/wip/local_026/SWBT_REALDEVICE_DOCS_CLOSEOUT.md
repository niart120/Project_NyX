...
serial backend を置き換えず、serial と swbt を並列に説明する。swbt backend は通常の PC Bluetooth 機能ではなく、Bumble から直接開く専用 USB Bluetooth adapter を使う前提で書く。swbt は通常依存なので `nyxpy-fw[swbt]` や `uv sync --extra swbt` は書かない。
...
- [ ] pytest の `swbt` marker と環境変数 gate を追加する。
