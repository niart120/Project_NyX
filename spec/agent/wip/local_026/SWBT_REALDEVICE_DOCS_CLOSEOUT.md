...
単体テストと dummy session では、Bluetooth adapter、pairing key、Switch 側接続状態、Joy-Con capability、stick Y 軸、short press の取りこぼし、close 後の入力残留を確認できない。利用者 docs も serial backend 前提のままでは、専用 adapter、key store、pair/reconnect/disconnect、GUI 操作、トラブル対応が分からない。
...
- 実機検証環境で通常依存の `swbt-python` が利用可能である。
- Switch、専用 USB Bluetooth adapter、キャプチャ入力、operator がそろっている。
- Pro Controller、Joy-Con L、Joy-Con R の実機検証が完了しない場合、`local_026` と親計画は complete へ移さない。

## 2. 対象ファイル

| ファイル | 変更種別 | 変更内容 |
|----------|----------|----------|
| `pyproject.toml` | 確認 | `swbt` pytest marker が登録済みであることを確認する |
| `conftest.py` | 変更 | swbt 実機テストは `NYX_REALDEVICE=1` と `NYX_SWBT=1` で `--realdevice` なしでも collection skip を解除できるようにする |
| `src/nyxpy/framework/core/hardware/swbt/session.py` | 変更 | controller method が awaitable を返した場合も session 内部で完了待ちする |
| `tests/unit/framework/hardware/swbt/test_session.py` | 変更 | awaitable を返す controller method が完了待ちされることを固定する |
| `tests/hardware/swbt_realdevice_support.py` | 新規 | 環境変数解釈、evidence writer、summary writer を実装する |
...
| `docs/user-guide/device-setup.md` | 変更 | swbt adapter、key store、pair/reconnect/disconnect 手順を追加する |
| `docs/user-guide/cli.md` | 変更 | `nyxpy swbt adapters/pair/reconnect/disconnect` と `nyxpy run --controller swbt` を追加する |
...
実機テストの制御は既存方針に合わせ、pytest option ではなく環境変数を使う。条件がない場合は `pytest.skip` する。標準入力待ちはしない。

### 証跡保存

実機検証ごとに evidence directory を作る。既定値は `tmp/hardware/swbt/<日時>/` とする。`NYX_SWBT_EVIDENCE_DIR` が指定された場合はその directory を使う。
...
`tmp/` は cleanup 対象であり、永続保存前提にしない。PR と complete 仕様には判定に必要な要約だけを載せる。
...
serial backend を置き換えず、serial と swbt を並列に説明する。swbt backend は通常の PC Bluetooth 機能ではなく、Bumble から直接開く専用 USB Bluetooth adapter を使う前提で書く。swbt は通常依存なので、利用者に swbt 用の extra 指定や追加同期手順を書かない。

### closeout 方針

`local_021` から `local_026` は、実装結果、検証コマンド、Pro Controller / Joy-Con L / Joy-Con R の実機結果、残課題が反映されるまで `wip` に残す。残課題が実装範囲外なら `spec/dev-journal.md` または新規 wip 仕様へ分離してから complete へ移す。
...
| 環境変数 | 型 | 既定値 | 説明 |
|----------|-----|--------|------|
| `NYX_REALDEVICE` | `str` | なし | `1` のとき実機テストを実行可能 |
...
| `NYX_SWBT_SHORT_PRESS_MS` | `str` | `16,33,50` | comma 区切り short press duration |
...
| `test_swbt_short_press_duration_realdevice` | trace は自動 | 必須 | 16ms、33ms、50ms の short press |

operator confirmation が必要なテストは `NYX_SWBT_OPERATOR_CONFIRMATION=1` がない場合 `pytest.skip` する。
...
| `device-setup.md` | 専用 adapter、adapter discovery、key store、pair/reconnect/disconnect |
| `cli.md` | `nyxpy swbt adapters/pair/reconnect/disconnect`、`nyxpy run --controller swbt` |
...
| テスト種別 | テスト名 | 検証内容 |
|------------|----------|----------|
| ユニット | `test_swbt_realdevice_options_from_environment` | 環境変数から実機テスト設定を構築する |
...
- [x] pytest の `swbt` marker と環境変数 gate を追加する。
- [x] evidence directory writer と operator confirmation writer を追加する。
- [x] adapter discovery 実機テストを追加する。
...
- [x] awaitable を返す controller method を session 内部で完了待ちする。
...
2026-07-10 時点で、実機なしで実装できる範囲を反映した。追加した実機テストは `@pytest.mark.realdevice` と `@pytest.mark.swbt` を持ち、`NYX_REALDEVICE=1`、`NYX_SWBT=1`、`NYX_SWBT_ADAPTER` が揃わない場合は実機操作へ進まない。
...
- `src/nyxpy/framework/core/hardware/swbt/session.py`: swbt-python 0.2 系の公開 API は同期呼び出しとして扱う。controller method が awaitable を返した場合も session 内部で完了待ちできるようにした。
...
現環境では `NYX_REALDEVICE`、`NYX_SWBT`、`NYX_SWBT_ADAPTER` が未設定のため、Pro Controller / Joy-Con L / Joy-Con R の実機検証は未実行である。従って `local_026` と親計画は complete へ移さない。
