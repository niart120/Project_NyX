...
既存の「接続 > キャプチャ入力 > 入力ソース」は camera/window 候補を入れ子メニューとして表示している。camera/window は候補選択が source type の切り替えを兼ねるため、この構造を維持する。直接接続型 capture source は device discovery 候補を持たないため、`カメラ` / `ウィンドウ` submenu と同じ階層に standalone の checkable action として追加する。
...
provider / profile / backend submenu は current source が `capture` のときだけ有効化する。`FPS` submenu は `camera` / `window` でのみ有効化し、`capture` では source cadence を `ponkan-python` / reader 設定へ委譲するため無効化または非表示にする。初期実装は既存 menu 構造への影響を抑えるため、無効化でよい。
...
| `キャプチャ` | `capture_source_type="capture"` | camera / window / capture provider / profile / ponkan settings |
| `キャプチャ Provider > ponkan` | `capture_provider="ponkan"` | `capture_source_type` |
| `キャプチャ Profile > n3dsxl` | `capture_device_profile="n3dsxl"` | `capture_source_type` |
| `Ponkan Backend > {backend}` | `ponkan_backend` | `capture_source_type` |
...
メニュー生成時に `ponkan` package を import してはならない。`ponkan-python` 未導入環境でも `キャプチャ` action と provider / profile / backend submenu は表示できる。依存不足は action の enabled 状態ではなく、capture preview 起動時の `preview_error` として扱う。
...
`MainWindow` に capture source 用 action group を追加する。

```python
class MainWindow(QMainWindow):
    capture_source_action: QAction | None
    capture_provider_menu: QMenu | None
    capture_profile_menu: QMenu | None
    ponkan_backend_menu: QMenu | None
    ponkan_backend_action_group: QActionGroup | None
```

`_populate_capture_source_type_menu()` は既存の `カメラ` / `ウィンドウ` submenu に加えて、`キャプチャ` checkable action を追加する。camera/window submenu 内の候補 action は従来どおり source type と選択対象を同時に更新する。`キャプチャ` action の checked 判定は `global_settings.get("capture_source_type") == "capture"` とする。
...
`_populate_capture_input_menu()` は provider / profile / ponkan backend submenu を作成し、`capture_provider`、`capture_device_profile`、`ponkan_backend` の choices を checkable action として表示する。provider / profile / backend action は source type を `capture` に切り替えず、各設定値だけを更新する。source 切替は、camera/window 候補選択または利用者が明示的に `キャプチャ` action を選ぶ操作に限定する。
...
| GUI | `test_connection_menu_lists_capture_source_action` | 「接続 > キャプチャ入力 > 入力ソース」に `キャプチャ` action が表示される |
| GUI | `test_connection_menu_applies_capture_source_setting` | `キャプチャ` action trigger で `capture_source_type` が `capture` になり、settings apply が呼ばれる |
...
| GUI | `test_connection_menu_has_ponkan_backend_submenu` | `Ponkan Backend` submenu に `auto` / `d3xx` / `d3xx-native` が表示され、current backend が checked になる |
...
- [ ] `MainWindow` 接続メニューに `キャプチャ` action を追加
- [ ] `MainWindow` 接続メニューに provider / profile / ponkan backend submenu を追加
