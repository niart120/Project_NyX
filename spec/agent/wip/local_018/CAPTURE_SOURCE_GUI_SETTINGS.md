...
既存の「接続 > キャプチャ入力 > 入力ソース」は camera/window 候補を入れ子メニューとして表示している。直接接続型 capture source は device discovery 候補を持たないため、同じ階層に checkable action として追加する。
...
`キャプチャ` action を実行すると次の settings だけを更新する。
...
`GuiAppServices.FRAME_SOURCE_SETTING_KEYS` に capture / ponkan keys を追加する。実行中に capture 設定を変更した場合は、既存どおり `deferred=True` として macro run 完了後に反映する。
...
capture source は camera/window discovery の対象ではないため、`未検出 (ダミーデバイス使用中)` とは表示しない。local_017 の `allow_dummy=True` fallback が発生した場合の user-facing 表示は別途 framework 側の `preview_error` / log に従う。
...
`_populate_capture_source_type_menu()` は `キャプチャ` checkable action を追加する。checked 判定は `global_settings.get("capture_source_type") == "capture"` とする。
...
`_populate_capture_input_menu()` は provider / profile / ponkan backend submenu を作成し、`capture_provider`、`capture_device_profile`、`ponkan_backend` の choices を checkable action として表示する。backend action は source type を `capture` に切り替えず、backend 値だけを更新する。source 切替は利用者が明示的に `キャプチャ` action を選ぶ操作に限定する。
