...
| manager close / release | Port の `close()` と `MacroRuntimeBuilder.shutdown()` |
...
| GUI の global manager 利用 | `capture_manager`, `serial_manager`, `global_settings`, `secrets_settings` を import | `MainWindow` は manager を所有せず、`GuiAppServices` 相当の composition object が discovery / Port factory / store を所有する |
...
3. GUI 起動時に `SettingsStore`、`SecretsStore`、`DeviceDiscoveryService`、Port factory を `GuiAppServices` 相当の composition object で生成する。
...
6. GUI / CLI cleanup は `runtime_builder.shutdown()` と logging close に寄せ、manager close / release 直呼びを残さない。
7. `SerialManager` / `CaptureManager` public class を削除し、必要な列挙 helper と device 生成は discovery / factory へ移す。
8. `NotificationHandlerPort` を参照するテストを `NotificationHandlerAdapter` へ移行し、alias を削除する。
9. `CancellationToken.request_stop()` の source 値を `request_stop` へ変更し、legacy 表現をテスト期待値に残さない。
...
| `DeviceDetectionTimeoutError` | device discovery が timeout 内に完了しない |
...
| ユニット | `test_device_discovery_replaces_managers` | `SerialManager` / `CaptureManager` を使わず device 検出結果を取得できる |
...
```powershell
rg "create_legacy_runtime_builder|nyxpy\.framework\.core\.singletons|from nyxpy\.framework\.core import singletons|NotificationHandlerPort|SerialManager|CaptureManager|serial_manager|capture_manager" src tests
...
- [ ] `singletons.py` を削除
- [ ] `SerialManager` / `CaptureManager` を DeviceDiscoveryService / Port factory へ置換
- [ ] `create_device_runtime_builder()` の manager / direct device 引数を削除
- [ ] `NotificationHandlerPort` alias を削除
- [ ] `CancellationToken.request_stop()` の legacy source 表現を削除
- [ ] GUI の singleton 依存を composition root 所有の discovery / Port factory / store へ移行
- [ ] GUI device settings tab の singleton import を削除
- [ ] manager の active device、dummy fallback、終了処理が実装コードに残っていないことを確認
