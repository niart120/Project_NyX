...
| `src/nyxpy/framework/core/hardware/serial_comm.py` | 変更 | `SerialManager` の現行責務を確認し、旧 singleton 前提や暗黙 dummy 選択が Runtime 方針と衝突しないか整理する |
| `src/nyxpy/framework/core/hardware/capture.py` | 変更 | `CaptureManager` の現行責務を確認し、旧 singleton 前提や暗黙 dummy 選択が Runtime 方針と衝突しないか整理する |
...
`SerialManager` / `CaptureManager` は、現時点では device 列挙、active device 切り替え、具象 device 生成を担う adapter として保持する。ただし cleanup 後の所有者は GUI / CLI の composition root であり、framework singleton の代替として共有状態を持たせない。

実装時に次を確認する。

| 観点 | 確認内容 |
|------|----------|
| 所有権 | GUI / CLI が manager instance を生成し、`create_device_runtime_builder()` に渡す。`singletons.py` 由来の manager を参照しない |
| Runtime との境界 | Runtime core は Port だけを扱う。manager は builder の device 解決時にのみ使い、`ExecutionContext` や `DefaultCommand` に露出させない |
| dummy device | `allow_dummy=False` では暗黙の dummy 選択が Runtime validation をすり抜けないことをテストする |
| 終了処理 | GUI cleanup は `MacroRuntimeBuilder.shutdown()` を中心に行う。manager の active device を GUI から直接閉じる経路を残す場合は、manager 所有 lifetime として理由を明記する |
| 将来分離 | device discovery / active device 管理を Port 生成責務から分ける必要が出た場合は、`DeviceDiscoveryService` など別仕様で扱う |
...
6. `NotificationHandlerPort` を参照するテストを `NotificationHandlerAdapter` へ移行し、alias を削除する。
7. `CancellationToken.request_stop()` の source 値を `request_stop` へ変更し、legacy 表現をテスト期待値に残さない。
...
| ユニット | `test_removed_framework_apis_have_no_call_sites` | 実装コードとテストコードに削除対象 API の参照が残っていない |
| ユニット | `test_device_builder_rejects_implicit_dummy_when_dummy_is_not_allowed` | manager の暗黙 dummy 選択が Runtime validation をすり抜けない |
...
```powershell
rg "create_legacy_runtime_builder|nyxpy\.framework\.core\.singletons|from nyxpy\.framework\.core import singletons|NotificationHandlerPort" src tests
...
- [ ] `singletons.py` を削除
- [ ] `NotificationHandlerPort` alias を削除
- [ ] `CancellationToken.request_stop()` の legacy source 表現を削除
- [ ] GUI の singleton 依存を composition root 所有の object へ移行
- [ ] GUI device settings tab の singleton import を削除
- [ ] Manager の所有権、dummy device、終了処理が旧 singleton 前提を引きずっていないことを確認
- [ ] framework / GUI / CLI / macros の依存方向ゲートを追加
