...
Runtime builder の構成起点で `serial` / `swbt` controller backend を選択し、`swbt` backend では serial protocol 生成を通らずに `SwbtControllerOutputPortFactory` を注入する。CLI には `nyxpy run --controller swbt` と `nyxpy swbt pair/reconnect/disconnect` を追加する。
...
| `nyxpy swbt disconnect` | 同一 factory が管理する cached session を閉じる CLI |
...
| pair/reconnect/disconnect CLI | 未導入 | `nyxpy swbt pair` / `reconnect` / `disconnect` で明示 lifecycle |
...
| `src/nyxpy/cli/swbt_cli.py` | 変更 | `pair`、`reconnect`、`disconnect` subcommand の処理を実装する |
| `src/nyxpy/__main__.py` | 変更 | `swbt pair/reconnect/disconnect` subparser を追加し、`swbt_cli.py` へ委譲する |
...
| `tests/unit/cli/test_swbt_cli.py` | 変更 | `pair`、`reconnect`、`disconnect` CLI を fake factory で検証する |
...
### `nyxpy swbt pair` / `reconnect` / `disconnect`
...
nyxpy swbt disconnect --adapter usb:0 --controller-type pro-controller
```

各 command は workspace settings を読み、CLI option で override して `SwbtControllerConfig` を作る。`--controller-type` を省略した場合は `controller.swbt.controller_type` を使う。`--key-store` を省略した場合は、解決済み controller type から `.nyxpy/swbt/<controller>-bond.json` を使う。`--adapter` を省略した場合は settings の `controller.swbt.adapter` を使う。settings と CLI の両方で adapter が空なら `NYX_SWBT_ADAPTER_NOT_SELECTED` とする。

CLI override は settings を書き換えない。

`disconnect` は factory-managed cached session だけを閉じる。別 process の接続、OS 側状態、Switch 側状態までは保証しない。cached session がなければ no-op とする。
...
| ユニット | `test_swbt_disconnect_cli_calls_factory_disconnect` | disconnect CLI が factory-managed session close を呼ぶ |
